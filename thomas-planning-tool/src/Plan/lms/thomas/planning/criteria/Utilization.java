...
public class Utilization extends AbstractCriterion {
	...
	public double getValue(Vector<TreeNode[]> paths, PlanHelperInterface helper, Calendar timeNow) {
		...
		// ManualPlanHelperInterface manualHelper =
		// helper.getManualPlanningHelperInterface();
		for (int i = 0; i < sr; i++) {
			...
			for (int j = 0; j < path.length; j++) {
				...
				// String
				// resourceName=resourceSimulator.getResourceDataModel().getResourceName();
				...
				for (Assignment assignment : node.getNodeAssignments()) {
					...
					System.out.println("                      Counter        " + counter + " sr " + sr + " pathLengh "
							+ path.length + " assi " + ass + " res " + resourceName + " task " + taskName);

					// double partialUtilizationTimeTemp=0;
					double partialUtilizationTimeTemp[] = { 0, 0 };

					// partialUtilizationTimeTemp=Simulation.simulationDemo1(resourceName,
					// taskName);
