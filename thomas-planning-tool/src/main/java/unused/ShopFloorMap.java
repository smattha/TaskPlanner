...
public class ShopFloorMap implements ShopFloorMapInterface {
    ...
    /**
     * Stores a bit more information (The id is also stored inside the object)
     * to provide faster search. However JVM specs internally specify that
     * identical strings are grouped together in memory, so that the memory
	...
	if (mapDimensions == null) {
	    throw new RuntimeException(msg
		    + "mapDimensions should never be null.");
	}
     ...
     */
    public ShopFloorMapInterface addRectangleObjectToMap(
     ...
     */
    public Boolean canAddRectangleObjectToMapWithoutOverlap(
	    RectangleObjectInterface aRectangle) {
	...
	for (RectangleObjectInterface anExistingRectangle : this.objectsMap
		.values()) {
     ...
     * (lms.robopartner.datamodel.map.RectangleObjectInterface)
     */
    public ShopFloorMapInterface removeRectangleObjectFromMap(
	    RectangleObjectInterface aRectangle) {
     ...
     * 
     * @see
     * lms.robopartner.datamodel.map.ShopFloorMapInterface#getMapDimension
     * ()
     */
    public Dimension getMapDimension() {
	Dimension aDimension = new Dimension(this.theShopfloorBounds.width,
		this.theShopfloorBounds.height);
	return aDimension;
    }
	...
	Dimension aNewDimension = new Dimension(this.getMapDimension().width,
		this.getMapDimension().height);
	ShopFloorMap aNewShopfloorMap = new ShopFloorMap(aNewDimension,
		this.getID());
     ...
     * 
     * @see
     * lms.robopartner.datamodel.map.ShopFloorMapInterface#setID(java.lang
     * .String)
     */
     ...
     * 
     * @see
     * lms.robopartner.datamodel.map.ShopFloorMapInterface#shallowCopy()
     */
    public ShopFloorMapInterface shallowCopy() {

	ShopFloorMap aNewShopfloorMap = new ShopFloorMap(
		this.getMapDimension(), this.getID());
