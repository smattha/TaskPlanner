...
public class ComparableTask extends TASK implements Comparable<TASK> {
	...
	int width = 0;
	int height = 0;
	if (aTask != null && aTask.getPROPERTIES() != null
		&& aTask.getPROPERTIES().getPROPERTY() != null) {
	    for (PROPERTY aProperty : aTask.getPROPERTIES().getPROPERTY()) {
		if (aProperty.getNAME() != null
			&& aProperty.getNAME().equals(
				MapToResourcesAndTasks.LENGTH_PROPERTY_NAME)) {
		    try {
		...
		if (aProperty.getNAME() != null
			&& aProperty.getNAME().equals(
				MapToResourcesAndTasks.WIDTH_PROPERTY_NAME)) {
		    try {
	...
	int lastCmp = Integer.compare(this.getSquareSize(),
		ComparableTask.getSquareSize(obj));
	lastCmp = (lastCmp != 0 ? lastCmp : Integer.compare(this.getWidth(),
		ComparableTask.getWidth(obj)));
	lastCmp = (lastCmp != 0 ? lastCmp : Integer.compare(this.getHeight(),
		ComparableTask.getHeight(obj)));
	if (obj != null) {
	    if (obj.getId() != null) {
		lastCmp = (lastCmp != 0 ? lastCmp : this.getId().compareTo(
			obj.getId()));
	    }
	    if (obj.getNAME() != null) {
		lastCmp = (lastCmp != 0 ? lastCmp : this.getNAME().compareTo(
			obj.getNAME()));
	    }
	    if (obj.getDESCRIPTION() != null) {
		lastCmp = (lastCmp != 0 ? lastCmp : this.getDESCRIPTION()
			.compareTo(obj.getDESCRIPTION()));
	    }
	}
