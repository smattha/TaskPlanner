...
public class EDD extends AbstractAlgorithm {
    protected EDD() {
    }
    ...
    public Vector<AssignmentDataModel> solve(Vector<TaskSimulator> pendingTaskSimulators, Vector<ResourceSimulator> idleResourceSimulators, Calendar timeNow, PlanHelperInterface helper) {
        // Sort the pendingTaskSimulators vector with the first due date
        ...
        if (pendingTaskSimulatorSize > 0) {
            ...
            for (int i = pendingTaskSimulatorArray.length; --i >= 0;) {
                ...
                for (int j = 0; j < i; j++) {
                    if (pendingTaskSimulatorArray[j].getDueDate().getTimeInMillis() > pendingTaskSimulatorArray[j + 1].getDueDate().getTimeInMillis()) {
        ...
        if (pendingTaskSimulatorArray != null) {
            for (int i = 0; i < pendingTaskSimulatorArray.length; i++) {
                ...
                Vector<ResourceSimulator> suitableResources = helper.getSuitableResourcesSimulatorForTask(taskSimulator);
                ...
                for (int j = 0; j < suitableResources.size(); j++) {
                    ...
                    if ((resourceSimulator.getResourceState().equals(ResourceSimulator.IDLE)) && (idleResourceSimulators.indexOf(resourceSimulator) != -1) && (helper.canResourceFullfilTheTaskUniterrupted(resourceSimulator, taskSimulator, timeNow))) {
                        potentialSuitableResourceSimulators.add(resourceSimulator);
                    }
                }
                // Shuffle the idle resources vector to choose one randomly
                // TODO: Maybe implement another algorithm to select the idle resource
                Collections.shuffle(potentialSuitableResourceSimulators);
                //Make an assignment if a resource is available and the task has fulfilled its precedence constrains
                for (ResourceSimulator resourceSimulator : potentialSuitableResourceSimulators) {
                    if (!helper.areAnyPendingPresendenceConstraintsForTaskOnResource(resourceSimulator, taskSimulator, timeNow)) {
                        // Creating the assignment
                        AssignmentDataModel assignment = new AssignmentDataModel(taskSimulator.getTaskDataModel(),
                                resourceSimulator.getResourceDataModel(),
                                timeNow,
                                helper.getSetUpTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, timeNow, null)
                                        + helper.getOperationTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, timeNow, null)
                                        + helper.getResourceDownTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, timeNow, null),
                                false, null);
