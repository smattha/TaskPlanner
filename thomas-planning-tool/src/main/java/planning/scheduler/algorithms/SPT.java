...
public class SPT extends AbstractAlgorithm {
	...
	public Vector<AssignmentDataModel> solve(Vector<TaskSimulator> pendingTaskSimulators,
		...
		// 1.2) if potential resources exist for these resource calculate the
		// processingTime = opperationalTime + setUpTime
		...
		// TODO : If i had a collection (vector) with all the task suitable resources it
		// would be quicker
		...
		while (!pendingTaskSimulators.isEmpty()) {
			...
			for (int i = 0; i < pendingTaskSimulators.size(); i++) {
				...
				Vector<ResourceSimulator> suitableResources = helper
						.getSuitableResourcesSimulatorForTask(currentTaskSimulator);
				...
				// If a resource is found then calculate it's processing time and store the
				// minimum values
				for (int j = 0; j < suitableResources.size(); j++) {
					ResourceSimulator resourceSimulator = suitableResources.get(j);
					if (!helper.areAnyPendingPresendenceConstraintsForTaskOnResource(resourceSimulator,
							currentTaskSimulator, timeNow)) {
						...
						if ((resourceSimulator.getResourceState().equals(ResourceSimulator.IDLE))
								...
								&& (helper.canResourceFullfilTheTaskUniterrupted(resourceSimulator,
										currentTaskSimulator, timeNow))) {
							// Checks if this suitable resource has the smallest processing time and replace
							// if applicable the
							...
							if (potentialSuitableResourceSimulator == null) {
								...
								potentialSuitableResourceSimulatorProcessingTime = helper
										.getSetUpTimeInMillisecondsForTaskOnResource(currentTaskSimulator,
												potentialSuitableResourceSimulator, timeNow, null)
										+ helper.getOperationTimeInMillisecondsForTaskOnResource(currentTaskSimulator,
												potentialSuitableResourceSimulator, timeNow, null);
							} else {// Perform a check
								long currentSuitableResourceSimulatorProcessingTime = helper
										.getSetUpTimeInMillisecondsForTaskOnResource(currentTaskSimulator,
												potentialSuitableResourceSimulator, timeNow, null)
										+ helper.getOperationTimeInMillisecondsForTaskOnResource(currentTaskSimulator,
												potentialSuitableResourceSimulator, timeNow, null);
								// TODO: what if the times are the same ? One thought is to check the resources
								// ... or
								// to choose randomly, and check of the overall value of the plan from the
				...
				if (potentialSuitableResourceSimulator != null) {
					// If it has a potential resource check if it has the smallest proccessing time
					// between al the tasks and replace if aplicable the
					...
					{
						// TODO: what if the times are the same ? One thought is to check the two tasks
						// times to other resources
						// and select the one that is most profitable. i.e. the one that has the bigger
						// difference between the task processing time
						// to this resource with the smallest processing time to other resources. I
						// thought the bigger because if we choose this then the loss will
						// be minimised. Another thought is to choose randomly, and check of the overall
						// value of the plan from the performance indicators.
			...
			if (potentialAssignmentTaskSimulator != null) {
				...
				AssignmentDataModel assignment = new AssignmentDataModel(
						potentialAssignmentTaskSimulator.getTaskDataModel(),
						potentialAssignmentResourceSimulator.getResourceDataModel(), timeNow,
						potentialAssignmentProcessingTime // operation time + set up time
								+ helper.getResourceDownTimeInMillisecondsForTaskOnResource(
										potentialAssignmentTaskSimulator, potentialAssignmentResourceSimulator, timeNow,
										null),
						false, null);
				...
				// Remove the task from the pending tasks vector (not pending any more an
