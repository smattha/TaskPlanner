...
public class LayerNode extends DefaultMutableTreeNode {
	...
	public LayerNode(HashMap<ResourceSimulator, Vector<TaskSimulator>> resourceSuitabilities,
			Vector<Assignment> assignments, Vector<ResourceSimulator> resources) {
	...
	private String makeId(Vector<Assignment> assignments) {
		...
		for (int i = 0; i < resourceIdsArray.length; i++) {
			...
			String taskId = resourceSimulatorToTaskSimulator.get(resourceIdToResourceSimulator.get(resourceId))
					.getTaskId();
	...
	public void createRandomChildNodeAssignment(int dh) {
		if (dh > 0) {
			if (ruag.hasNext()) {
				...
				// ...
				Vector<LayerNode> childrenCapableOfProducingAlternativePaths = new Vector<LayerNode>(); // Used for DH
																										// >1
				for (int i = 0; i < children.size(); i++) {
					LayerNode child = (LayerNode) children.get(i);
					if (child.canProduceLayeredAlternatives(dh - 1)) {
				...
				if (childrenCapableOfProducingAlternativePaths.size() == 0)
					...
					LayerNode child = childrenCapableOfProducingAlternativePaths
							.get(r.nextInt(0, childrenCapableOfProducingAlternativePaths.size() - 1));
					child.createRandomChildNodeAssignment(dh - 1);
					// PlanSimulator.printDebugMessage(" "+
					// child.getNodeAssignments().get(0).getTask().getTaskId()+" ");
				}
	...
	private void createRandomSolution() {
		if (ruag.hasNext()) {
			...
			if (childrenCapableOfProducingAlternativePaths.size() == 0)
				...
				LayerNode child = childrenCapableOfProducingAlternativePaths.get(0);
				// PlanSimulator.printDebugMessage("
				// "+child.nodeAssignments.get(0).getTask().getTaskId()+" ");
				child.createRandomSolution();
			} else {
				LayerNode child = childrenCapableOfProducingAlternativePaths
						.get(r.nextInt(0, childrenCapableOfProducingAlternativePaths.size() - 1));
				...
				for (int i = 0; i < resourcesSize; i++) {
					...
					if (clearedResourceTasks.size() != 0) {
						...
						if (i + 1 == resourcesSize)// Last assignment
						{
							...
							if (assignments.get(newPotentialAssignemntKey) != null) {
								...
								for (int j = 0; j < idleResourcesPossitionInPotentialAssignmentArray.size(); j++) {
									Vector<Integer> resourceSuitabilities = resourceToTasksSuitabilities
											.get(idleResourcesPossitionInPotentialAssignmentArray.get(j));
									for (int k = 0; k < resourceSuitabilities.size(); k++) {
										if (assignedTasks.indexOf(resourceSuitabilities.get(k)) == -1) {
								...
								if (!allResourcesHaveTasks && !allTasksAreAssigned
										&& pendingTasksCanBePerformedOnIdleResources) {
									// First delete the path that reached us
									// here ...
									assignmentsToTasks.get(assignmentKey).get(resourcePossition)
											.remove(new Integer(task));

									// Not valid solution so BACK BACK
									Integer previousResourcePossition = null;
									if (resourceToAssignmentOrder.size() > 1)
										previousResourcePossition = resourceToAssignmentOrder
												.get(resourceToAssignmentOrder.size() - 2);
									else {
									...
									if (previousResourcePossition != null) {
										...
										HashMap<Integer, Vector<Integer>> previousResourcesToTasks = assignmentsToTasks
												.get(previousKey);
										Vector<Integer> resourceTasks = previousResourcesToTasks
												.get(previousResourcePossition);
						...
						for (; tempI < sampleResource.length; tempI++) {
							if (futureSolutionSpace.get(sampleResource[tempI]) == null
									|| futureSolutionSpace.get(sampleResource[tempI]).size() != 0) {
							...
							if (i + 1 == resourcesSize) {
								...
								if (assignments.get(assignmentKey) != null) {
									...
									if (!allResourcesHaveTasks && !allTasksAreAssigned
										...
										// here ...
										assignmentsToTasks.get(assignmentKey).get(resourcePossition)
												.remove(assignment[resourcePossition]);
