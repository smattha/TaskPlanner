...
public class IMPACT extends AbstractAlgorithm {
	...
	public class DecisionPointOrderedSolutionsAndUtilitiesCarrier {
		...
		DecisionPointOrderedSolutionsAndUtilitiesCarrier(HashMap<TreeNode[], Double> solutionUtilities,
				Calendar timeNow, PlanHelperInterface helper) {
		...
		public Vector<HashMap<String, Object>> getOrderedAlternatives() {
			if (orderedAlternatives.size() == 0) {
				...
				List<Map.Entry<TreeNode[], Double>> list = new Vector<Map.Entry<TreeNode[], Double>>(
						solutionUtilities.entrySet());
						...
						for (int i = 0; i < assignmentsVector.size(); i++) {
							...
							AssignmentDataModel assignment = new AssignmentDataModel(taskSimulator.getTaskDataModel(),
									...
									helper.getSetUpTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator,
											timeNow, assignmentsVector)
											+ helper.getOperationTimeInMillisecondsForTaskOnResource(taskSimulator,
													resourceSimulator, timeNow, assignmentsVector)
	...
	/**
	 * DecisionPointOrderedUtilityListeners is a Vector of {@link ActionListener}
	 * for this impact.
	 */
	private final Vector<ActionListener> decisionPointOrderedUtilityListeners = new Vector<ActionListener>();
	/*
	 * Custom implementation for dynamic suitabilities. Suitabilities can only be
	 * removed so at the beginning of the plan all should exist
	 */
	...
	/**
	 * 1) Removing assigned tasks 2) Removing tasks that cannot be performed during
	 * the resource availability time (tasks are non preemptive) 3) Removing tasks
	 * that has precedence constrains 4) Checking for next task in chain constrains
	 */
	private Vector<TaskSimulator> initializeSuitableTasksForRootNode(Vector<TaskSimulator> suitableTasksSimulators,
			Vector<TaskSimulator> pendingTaskSimulators, ResourceSimulator resourceSimulator, Calendar timeNow,
			PlanHelperInterface helper) {
		...
		for (int i = 0; i < suitableTasksSimulators.size(); i++) {
			...
			if (pendingTaskSimulators.indexOf(taskSimulator) != -1
					...
					&& !helper.areAnyPendingPresendenceConstraintsForTaskOnResource(resourceSimulator, taskSimulator,
							timeNow)) {
				...
				if (dynamicResourceTaskSuitabilityAuthorities.size() > 0) {
					...
					for (DynamicResourceTaskSuitabilityAuthorityInterface dynamicResourceTaskSuitabilityAuthority : dynamicResourceTaskSuitabilityAuthorities) {
						if (!dynamicResourceTaskSuitabilityAuthority.isSuitabilityValid(resourceSimulator,
								taskSimulator, timeNow, helper)) {
	...
	private void printSolutionSpace(HashMap<TreeNode[], Vector<TreeNode[]>> solutionSpace) {
		...
		for (Iterator<TreeNode[]> iterator = solutionSpace.keySet().iterator(); iterator.hasNext();) {
				...
				for (int j = 0; j < path.length; j++) {
					...
					for (int k = 0; k < assignments.size(); k++) {
						...
						if (j <= this.decisionHorizon) {
							PlanSimulator.printDebugMessage("\t(" + assignment.getResource().getResourceId() + ":"
			...
			// PlanSimulator.printlnDebugMessage("\tASSIGNMENT "+(j+1)+" :
			// "+nodeAssignment.getResource().getResourceId()+" ->
			// "+nodeAssignment.getTask().getTaskId());
			// }
			// }
	...
	private void printSolutionUtilities(HashMap<TreeNode[], Double> solutionUtilities,
			HashMap<TreeNode[], double[]> solutionValues) {
		int s = 0;
		for (Iterator<TreeNode[]> iterator = solutionValues.keySet().iterator(); iterator.hasNext();) {
			...
			for (int i = 0; i < criteria.length; i++) {
				try {
					criteriaString += " \"" + criteria[i].getCriterionName() + "\":"
							+ formatNumber.valueToString(new Double(samples[i]));
			...
			try {
				PlanSimulator.printlnDebugMessage(
						"ALTERNATIVE " + (s) + criteriaString + " UTILITY : " + formatNumber.valueToString(utility));
			} catch (ParseException e) {
				e.printStackTrace();
			}
			...
			for (int i = 0; i < alternative.length; i++) {
				...
				for (int j = 0; j < nodeAssignments.size(); j++) {
					Assignment nodeAssignment = nodeAssignments.get(j);
					PlanSimulator.printlnDebugMessage(
							"\tASSIGNMENT " + (j + 1) + " : " + nodeAssignment.getResource().getResourceId() + " -> "
									+ nodeAssignment.getTask().getTaskId());
				}
			}
		}
	...
	public void setDecisionPointAssignmentsConsumer(
			DecisionPointAssignmentsConsumerInterface decisionPointAssignmentsConsumer) {
	...
	public void setDynamicResourceTaskSuitabilityAuthorities(
			Vector<DynamicResourceTaskSuitabilityAuthorityInterface> dynamicResourceTaskSuitabilityAuthorities) {
	...
	public Vector<AssignmentDataModel> solve(Vector<TaskSimulator> pendingTaskSimulators,
		...
		for (int i = 0; i < idleResourceSimulators.size(); i++) {
			...
			Vector<TaskSimulator> suitableTasksSimulators = helper
					.getSuitableTasksSimulatorForResource(resourceSimulator);
			Vector<TaskSimulator> cleanResourceTasks = initializeSuitableTasksForRootNode(suitableTasksSimulators,
					pendingTaskSimulators, resourceSimulator, timeNow, helper);
		...
		LayerNode lastCreated = null; // used to check if hole solution space was created before the MNA for the
										// current decision point
		for (long i = 0; i < this.getMNA(); i++) {
			// Making the other nodes
			// PlanSimulator.printDebugMessage("\n");
			// PlanSimulator.printDebugMessage("IMPACT : Building alternative
			// "+(i+1)+"/"+IMPACT.maximumNumberOfAlternatives+" ...");
			// PlanSimulator.printDebugMessage("\t");

			// TODO: Check assignments validity by the dynamic authority ... if not
			// recalculate:
			...
			if (this.dynamicAssignmentsAuthorityInterfaces != null && !rootNode.isLeaf()) {
				...
				while (!areAssignmentsValid) {
					...
					if (dhNode != null && dhNode.getNodeAssignments() != null
							&& dhNode.getNodeAssignments().size() > 0) {
						...
						for (Assignment assignment : assignments) {
							...
							long durationInMilliseconds = helper.getOperationTimeInMillisecondsForTaskOnResource(
									assignment.getTask(), assignment.getResource(), timeNow,
									new Vector<LayerNode.Assignment>());
							AssignmentDataModel assignmentDataModel = new AssignmentDataModel(task, resource, timeNow,
									durationInMilliseconds, false, null);
						...
						for (DynamicAssignmentsAuthorityInterface assignmentsAuthorityInterface : dynamicAssignmentsAuthorityInterfaces) {
							if (!assignmentsAuthorityInterface.areAssignmentsValid(assignmentDataModels, timeNow,
									helper)) {
								// RECALCULATE
			...
			// 3) Convert Vector<AssignmentDataModel> to LayerNode and append it to the
			// rootNode's DH assignment layerNode (in position i).
			// Sample LayerNode suitabilities and resources must be NOT NULL empty hashmap,
			// vector accordingly
			// ATTENTION !!! A LayerNode represent a DH layer so:
			// i) Assignments on the same time must be added on the same Layer
			// ii) Subsequent assignments must have their own LayerNode and linked to
			// previous
			...
			if (!rootNode.isLeaf()) {
				...
				if (dhNode.equals(lastCreated)) {
					...
					for (int j = 0; j < this.getSR(); j++) {
						...
						Vector<Vector<AssignmentDataModel>> timeOrderedAssignments = IMPACT
								.getTimeOrderedAssignmentsAfterTimeNow(sample, timeNow);

						// ----------------START create LayerNodes and add them to DH
						// -------------------------
						LayerNode tempLayerNodeToAccessAssignmentConstructor = new LayerNode(
								new HashMap<ResourceSimulator, Vector<TaskSimulator>>(), new Vector<Assignment>(),
						...
						for (Vector<AssignmentDataModel> currentAssignments : timeOrderedAssignments) {
							...
							for (AssignmentDataModel anAssignmentDataModel : currentAssignments) {
								...
								Assignment anAssignment = tempLayerNodeToAccessAssignmentConstructor.new Assignment(
										helper.getResourceSimulatorFromResourceDataModel(
												anAssignmentDataModel.getResourceDataModel()),
										helper.getTaskSimulatorFromTaskDataModel(
												anAssignmentDataModel.getTaskDataModel()));
								currentLayerAssignments.add(anAssignment);
							}
							// createNewNode
							LayerNode currentLayerNode = new LayerNode(
									new HashMap<ResourceSimulator, Vector<TaskSimulator>>(), currentLayerAssignments,
									new Vector<ResourceSimulator>());
		...
		for (Iterator<TreeNode[]> iterator = solutionValues.keySet().iterator(); iterator.hasNext();) {
			...
			for (int i = 0; i < criteria.length; i++) {
				if (criteria[i].isBenefit()) {
					...
					normilizedCriteriaValues[i] = (1d / ((solutionValues.size()) - 1d))
							* (1d - (criteriaValues[i] / criteriaSums[i]));
					if (criteriaSums[i] == 0) {
		...
		if (decisionPointOrderedUtilityListeners.size() > 0) {
			for (int i = 0; i < decisionPointOrderedUtilityListeners.size(); i++) {
				...
				DecisionPointOrderedSolutionsAndUtilitiesCarrier decisionPointSolutionAndUtilityCarrier = new DecisionPointOrderedSolutionsAndUtilitiesCarrier(
						solutionUtilities, timeNow, helper);
				ActionEvent event = new ActionEvent(decisionPointSolutionAndUtilityCarrier,
						ActionEvent.ACTION_PERFORMED, "New Assignments");
		 ...
		 * nf.valueToString(bestUtilityScore) ); //PlanSimulator.printlnDebugMessage
		 * ("SR\t"+IMPACT.getSR()+"\tTASK\t"+taskSimulator .getTaskId()+"\tRESOURCE\t"
		 * +resourceSimulator.getResourceId()+"\tUTILITY\t"
		 * +nf.valueToString(bestUtilityScore)); for(int j=1; j<assignmentNodes.length;
		 * j++) { LayerNode assignmentNode = (LayerNode)assignmentNodes[j];
		 * Vector<Assignment> assignmentsVector = assignmentNode.getNodeAssignments();
		 * for (int i = 0; i < assignmentsVector.size(); i++) { Assignment
		 * layerAssignment = assignmentsVector.get(i); TaskSimulator taskSimulator =
		 * layerAssignment.getTask(); ResourceSimulator resourceSimulator =
		 * layerAssignment.getResource(); if(j+1!=assignmentNodes.length) PlanSimulator
		 * .printDebugMessage(resourceSimulator.getResourceId()+"\t"
		 * +taskSimulator.getTaskId()+"\t"); else PlanSimulator.printDebugMessage
		 * (resourceSimulator.getResourceId()+"\t"+taskSimulator.getTaskId()); } }
		 * PlanSimulator.printlnDebugMessage("\t"+IMPACT.getMNA()+"\t"+nf. valueToString
		...
		// appropriate
		Vector<AssignmentDataModel> assignments = decisionPointAssignmentsConsumer.consumeDecisionPointAssignments(
				timeNow, helper, assignmentsVector, solutionValues.get(assignmentNodes));
		// Update the kpis
	 ...
	 * @return
	 */
	private Vector<AssignmentDataModel> runSamplingImpact(PlanHelperInterface planHelperInterface, Calendar timeNow,
			Vector<Assignment> decisionHorizonAssignments) {
		int dh = 1;
		int mna = 1;
		int sr = 0;
		...
		Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints = planHelperInterface
				.getTaskPrecedenceConstraints();
		...
		PlanningInputDataModel planningInputDataModel = new PlanningInputDataModel(resources, tasks,
				precedenceConstraints, tasksSuitableResources, workcenters, jobs, toolTypes, tools, mobileResourceTypes,
				mobileResources);
		...
		for (AssignmentDataModel unlockedAssignmentDataModel : tempLockedAssignments) {
			AssignmentDataModel lockedAssignmentDataModel = new AssignmentDataModel(
					unlockedAssignmentDataModel.getTaskDataModel(), unlockedAssignmentDataModel.getResourceDataModel(),
					timeNow, unlockedAssignmentDataModel.getDurationInMilliseconds(), lock,
					unlockedAssignmentDataModel.getProperties());
			lockedAssignments.add(lockedAssignmentDataModel);
		}
		for (Assignment aDHAssignment : decisionHorizonAssignments) {
			AssignmentDataModel lockedAssignmentDataModel = new AssignmentDataModel(
					aDHAssignment.getTask().getTaskDataModel(), aDHAssignment.getResource().getResourceDataModel(),
					timeNow,
					planHelperInterface.getOperationTimeInMillisecondsForTaskOnResource(aDHAssignment.getTask(),
							aDHAssignment.getResource(), timeNow, new Vector<LayerNode.Assignment>()),
					lock, null);
			lockedAssignments.add(lockedAssignmentDataModel);
		}
		...
		mptIMPACT.setSR(sr);
		mptIMPACT.setCriteria(new AbstractCriterion[] { dummyCriterion });
	 ...
	 * @return
	 */
	private static final Vector<Vector<AssignmentDataModel>> getTimeOrderedAssignmentsAfterTimeNow(
			Vector<AssignmentDataModel> assignments, Calendar timeNow) {
	...
	public void setDynamicAssignmentsAuthorityInterfaces(
			Vector<DynamicAssignmentsAuthorityInterface> dynamicAssignmentsAuthorityInterfaces) {
