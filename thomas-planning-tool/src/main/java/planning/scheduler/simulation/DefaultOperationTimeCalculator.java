...
public class DefaultOperationTimeCalculator implements OperationTimeCalculatorInterface {
    ...
    public long getOperationTimeInMillisecondsForTaskOnResource(TaskSimulator taskSimulator, ResourceSimulator resourceSimulator, Calendar timeNow, PlanHelperInterface helperInterface, Vector<Assignment> assignments) {
        ...
        Vector<TaskSuitableResourceDataModel> taskSuitableResourceDataModelVector = helperInterface.getAllSuitabilities();
        ...
        if (time == Long.MIN_VALUE) {
            System.err.println("Warning: Task '" + taskSimulator.getTaskDataModel().getTaskName() + "' has not operation time set. Using 1ms");
            time = 1;
        }
        return time;
    }
