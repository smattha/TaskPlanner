...
public class PoolOfPrereleasedAssignmentsSimulator {
	...
	public AssignmentDataModel getNextPrereleaseAssignment(ResourceDataModel resource, Calendar currentTime) {
		...
		for (int i = 0; i < this.assignments.size(); i++) {
			...
			if (currentPrereleaseAssignment.getResourceDataModel().equals(resource))// Check for the resource
																					// assignments
			{
				if (currentTime.getTimeInMillis() <= currentPrereleaseAssignment.getTimeOfDispatch().getTimeInMillis())// Check
																														// for
																														// the
																														// time
																														// of
																														...
																														// assignment
				{
					...
					} else if (nextPrereleaseAssignment.getTimeOfDispatch()
							.getTimeInMillis() > currentPrereleaseAssignment.getTimeOfDispatch().getTimeInMillis()) {
						nextPrereleaseAssignment = currentPrereleaseAssignment;
					}
				}
			}
