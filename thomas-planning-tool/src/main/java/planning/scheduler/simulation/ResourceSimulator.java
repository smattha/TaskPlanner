...
public class ResourceSimulator {
	...
	public boolean isResourceDown(Calendar currentTime) {
		...
		for (int i = 0; i < exceptionPeriods.size(); i++) {
			...
			if (currentTimeInMilliseconds > fromDateInMilliseconds) {
				...
				if (periodInMilliseconds != 0) {
					long quotient = differenceFromDateToCurrentDateInMilliseconds / periodInMilliseconds + 1;// We add
																												// one
																												// because
																												// we
																												// want
			...
			} else // currentTimeInMilliseconds == fromDateInMilliseconds thus the resource is
					// about to get down
			{
				return false;
			}
	...
	protected Calendar getNextNonWorkingDate(Calendar date) {
		if (isResourceDown(date)) {
			...
			if (nextNonWorkingDateDueToForcedWorkingPeriods != null && nextWorkingDateDueToNonWorkingPeriods != null
					&& nextNonWorkingDateDueToForcedWorkingPeriods
				...
				// ... no check for non working periods means that here there might be bug in
				// the future
			...
			// else
			{
				...
				Calendar nextNonWorkingDateDueToForcedWorkingPeriods2 = getNextNonWorkingDateFromForcedWorkingDates(
						nextNonWorkingDateDueToNonWorkingPeriods);
		...
		// else
		{
			if (isDateInsideTheResoureNonWorkingPeriods(date)) {
				// Must check if the next day the resource will go down due to forced working
				// periods
				// is smallest from the day that the resource will be up due to non
				// workingperiod.
				// This means that the next date the resource will go down due to forced working
				// periods is allready up
				// so next non working period is the next time the resource will be down also
			...
			// else
			{
				...
				if (isDateInsideTheResoureForcedWorkingPeriods(nextNonWorkingDateDueToNonWorkingPeriods))// Checking if
																											// the next
																											// working
																											// date due
																											// to non
	...
	private Calendar getNextNonWorkingDateFromNonWorkingDates(Calendar date) {
		...
		while (isDateInsideTheResoureNonWorkingPeriods(copyOfDateToCheck)) {
			...
			// So must check when this period ends and then find the next non working
			// copyOfDateToCheck
			...
			if (date.getTimeInMillis() > startPeriodDate.getTimeInMillis()) {
				...
				if (periodInMilliseconds != 0) {
					...
					if (date.getTimeInMillis() <= startPeriodDate.getTimeInMillis() + periodTimes * periodInMilliseconds
						...
						// Means we are inside a forced working period so the end might be the next non
						// working date
						nextNonWorkingDateInMilliseconds = startPeriodDate.getTimeInMillis()
								+ periodTimes * periodInMilliseconds + durationInMilliseconds;
					} else {
						// We are outside the non working period so we must add one period and then is
						// the next period's end date
						periodTimes++;
						nextNonWorkingDateInMilliseconds = startPeriodDate.getTimeInMillis()
								+ periodTimes * periodInMilliseconds + durationInMilliseconds;
					}
