...
public class PlanSimulator implements PlanHelperInterface, ManualPlanHelperInterface {
	...
	private Vector<TaskPrecedenceConstraintDataModel> precedenceConstraintDataModelVector = null; // Should change to a
																									// PoolOfConstraints
																									// class
	private Vector<TaskSuitableResourceDataModel> taskSuitableResourceDataModelVector = null; // Should change to a
																								// PoolOfConstraints
																								// class ??
	...
	public PlanSimulator(Vector<WorkcenterDataModel> workcenterDataModelVector, Vector<JobDataModel> jobDataModelVector,
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraintDataModelVector,
			Vector<TaskSuitableResourceDataModel> taskSuitableResourceDataModelVector, Calendar startTime) {
	...
	public PlanSimulator(Vector<WorkcenterDataModel> workcenterDataModelVector, Vector<JobDataModel> jobDataModelVector,
			Vector<AssignmentDataModel> assignmentDataModelVector,
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraintDataModelVector,
			Vector<TaskSuitableResourceDataModel> taskSuitableResourceDataModelVector, Calendar startTime) {
		this.startTime = startTime;
		this.workcenterDataModelVector = workcenterDataModelVector;
	...
	/**
	 * Perform the full simulation. The simulation will end until all tasks are
	 * released into resources to conform assignments.
	 */
	public Vector<AssignmentDataModel> simulate(PlanEndRule planEndRule) {
		...
		Vector<AssignmentDataModel> allPrereleasedAssignments = this.prereleasedAssignments
				.getPrereleaseAssignmentDataModelVector();
		for (int i = 0; i < allPrereleasedAssignments.size(); i++) {
			...
			if (timeNow.getTimeInMillis() >= allPrereleasedAssignments.get(i).getTimeOfDispatch().getTimeInMillis()) {
				...
				// replace the old
				AssignmentDataModel newPrereleasedAssignment = new AssignmentDataModel(
						prereleasedAssignment.getTaskDataModel(), prereleasedAssignment.getResourceDataModel(),
						prereleasedAssignment.getTimeOfDispatch(),
						getSetUpTimeInMillisecondsForTaskOnResource(
								tasks.getTaskSimulator(prereleasedAssignment.getTaskDataModel()),
								resources.getResourceSimulator(prereleasedAssignment.getResourceDataModel()),
								prereleasedAssignment.getTimeOfDispatch(), null)
								+ getOperationTimeInMillisecondsForTaskOnResource(
										tasks.getTaskSimulator(prereleasedAssignment.getTaskDataModel()),
										resources.getResourceSimulator(prereleasedAssignment.getResourceDataModel()),
										prereleasedAssignment.getTimeOfDispatch(), null)
								+ getResourceDownTimeInMillisecondsForTaskOnResource(
										...
										resources.getResourceSimulator(prereleasedAssignment.getResourceDataModel()),
										prereleasedAssignment.getTimeOfDispatch(), null),
						true, prereleasedAssignment.getProperties());
				...
				if (timeNow.getTimeInMillis() >= allPrereleasedAssignments.get(i).getTimeOfDispatch().getTimeInMillis()
						+ allPrereleasedAssignments.get(i).getDurationInMilliseconds()) {
		...
		while (interupts.size() > 0) {
			...
			// Currently only used when an assignment finishes to set the resources state
			// back to IDLE and the tasks to COMPLETED state
			for (Interupt interupt : interupts) {
				if (interupt.getMessage().equals(Interupt.ASSIGNMENTS_FINISHED)) {
					...
					Vector<AssignmentDataModel> assignmentsFinished = (Vector<AssignmentDataModel>) interupt
							.getSource();
					...
					for (int i = 0; i < lockedAssignments.size(); i++) {
						...
						// Checking the finish time ...
						if (timeNow.getTimeInMillis() >= lockedAssignment.getTimeOfDispatch().getTimeInMillis()
								+ lockedAssignment.getDurationInMilliseconds()) {
			...
			// TODO: Maybe implement better Interrupt class so that to plan only
			// "interrupted" workcenters
			...
			for (int i = 0; i < workcenterDataModelVector.size(); i++) {
				...
				Vector<TaskSimulator> pendingTasks = this
						.findSuitableTaskSimulatorsForWorkcenterDataModelFromAvailableTaskSimulators(
								tasks.getPendingTaskSimulatorVector(timeNow), currentWorkcenterDataModel);
				Vector<ResourceSimulator> idleResource = resources
						.getIdleResourceSimulatorVector(currentWorkcenterDataModel);

				// Choosing the algorithm and performing the assignments
				AbstractAlgorithm algorithm = this.getAlgorithmFactory()
						.getAlgorithmInstance(currentWorkcenterDataModel.getAlgorithm());
				Vector<AssignmentDataModel> workcenterAssignments = algorithm.solve(pendingTasks, idleResource, timeNow,
						this);
...
//          Notify the engine for the new assignments
		...
		// System.out.println("assignmentsDataModelVector.size() :
		// "+assignmentsDataModelVector.size());
		for (int i = 0; i < lockedAssignments.size(); i++) {
			AssignmentDataModel lockedAssignment = lockedAssignments.get(i);
			for (int j = 0; j < assignmentsDataModelVector.size(); j++) {
				...
				if (currentAssignment.getTaskDataModel().equals(lockedAssignment.getTaskDataModel())) {
					...
					// replace the old
					AssignmentDataModel newLockedAssignment = new AssignmentDataModel(
							currentAssignment.getTaskDataModel(), currentAssignment.getResourceDataModel(),
							currentAssignment.getTimeOfDispatch(),
							getSetUpTimeInMillisecondsForTaskOnResource(
									tasks.getTaskSimulator(currentAssignment.getTaskDataModel()),
									resources.getResourceSimulator(currentAssignment.getResourceDataModel()),
									currentAssignment.getTimeOfDispatch(), null)
									+ getOperationTimeInMillisecondsForTaskOnResource(
											tasks.getTaskSimulator(currentAssignment.getTaskDataModel()),
											resources.getResourceSimulator(currentAssignment.getResourceDataModel()),
											currentAssignment.getTimeOfDispatch(), null)
	...
	private void printDebugInfoForAssignment(AssignmentDataModel assignment) {
		if (DEBUG) {
			System.out.println("\tNEW ASSIGNMENT FOR TASK : " + assignment.getTaskDataModel().getTaskId()
					+ " RESOURCE : " + assignment.getResourceDataModel().getResourceId() + " ON : "
					+ assignment.getTimeOfDispatch().getTime());
		}
	}
	...
	private void printDebugInfoForInterupt(Vector<Interupt> interupts) {
		if (DEBUG) {
			for (Interupt interupt : interupts) {
				...
				if (interupt.getMessage().equals(Interupt.PLAN_START)) {
					...
					for (int i = 0; i < ((Vector<AssignmentDataModel>) interupt.getSource()).size(); i++) {
						interupDetails = interupDetails + "\n \t\t\t task = "
								+ ((Vector<AssignmentDataModel>) interupt.getSource()).get(i).getTaskDataModel()
										.getTaskId()
								+ " resource = " + ((Vector<AssignmentDataModel>) interupt.getSource()).get(i)
										.getResourceDataModel().getResourceId();
					}
				...
				System.out.println("NEW INTERUPT AT : " + interupt.getDate().getTime() + " REASON : "
						+ interupt.getMessage() + interupDetails);
			}
		}
	}
	...
	public boolean isTaskSuitableForResource(TaskSimulator taskSimulator, ResourceSimulator resourceSimulator) {
		for (int i = 0; i < taskSuitableResourceDataModelVector.size(); i++) {
			...
			if (taskSimulator.getTaskDataModel().equals(taskSuitableResource.getTaskDataModel())
					&& resourceSimulator.getResourceDataModel().equals(taskSuitableResource.getResourceDataModel()))
				return true;
		}
		return false;
	}
	...
	public boolean areAnyPendingPresendenceConstraintsForTaskOnResource(ResourceSimulator resourceSimulator,
			TaskSimulator taskSimulator, Calendar currentTime) {
		// MUST CHECK THE RESOURCE PREVIOUS TASKS IF HAS PENDING CONSTRAINTS ... TASK IN
		// CHAIN WITH RESOURCE UNAVAILABLE ...
		AssignmentDataModel previousAssignment = assignments
				.getLastAssignmentOfResource(resourceSimulator.getResourceDataModel());
		if (previousAssignment != null) {
			// Check to see if previous assignment had next task in chain with resource
			// unavailable
			for (int i = 0; i < precedenceConstraintDataModelVector.size(); i++) {
				...
				if (constraint.getPreconditionTask().equals(previousAssignment.getTaskDataModel())) {
					...
					if (constraint.getIsConstraintTypeNextTaskInChain()
							&& constraint.getIsResourceUnavailableBetweenTasks()
							&& !constraint.getPostconditionTask().equals(taskSimulator.getTaskDataModel())) {
						return true;
					}
				}
			}
		...
		for (int i = 0; i < precedenceConstraintDataModelVector.size(); i++) {
			...
			if (constraint.getPostconditionTask().equals(taskSimulator.getTaskDataModel())) {
				...
				if (assignment != null) {
					...
					// Checking if the constraint is of type next task in chain ... and check the
					// assignment resource with the current resource
					if (constraint.getIsConstraintTypeNextTaskInChain()) {
	...
	public long getOperationTimeInMillisecondsForTaskOnResource(TaskSimulator taskSimulator,
			ResourceSimulator resourceSimulator, Calendar timeNow, Vector<Assignment> assignments) {
		return operationTimeCalculator.getOperationTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator,
				timeNow, this, assignments);
	}
	...
	public Vector<ResourceSimulator> getSuitableResourcesSimulatorForTask(TaskSimulator taskSimulator) {
		...
		for (int i = 0; i < taskSuitableResourceDataModelVector.size(); i++) {
			...
			if (taskSuitableResource.getTaskDataModel().equals(taskSimulator.getTaskDataModel())) {
				ResourceSimulator resourceFound = resources
						.getResourceSimulator(taskSuitableResource.getResourceDataModel());
	...
	public long getSetUpTimeInMillisecondsForTaskOnResource(TaskSimulator taskSimulator,
			ResourceSimulator resourceSimulator, Calendar currentTime, Vector<Assignment> currentLevelAssignments) {
		...
		for (int i = 0; i < assignmentDataModelVector.size(); i++) {
			...
			if (currentAssignmentFinishTimeInMilliseconds <= currentTime.getTimeInMillis()) {
				if (assignmentDataModel.getResourceDataModel().getResourceId().equals(resourceDataModel.getResourceId())
					...
					latestAssignmentFinishTimeInMilliseconds = currentAssignmentFinishTimeInMilliseconds;
						...
						.equals(resourceDataModel.getResourceId())) {
					// Check the latest assignment dispatch time plus the operational time plus the
					// set up time
		...
		if (latestAssignmentDataModel == null) {
			...
			fromCode = getSetUpCodeForTaskOnResource(
					tasks.getTaskSimulator(latestAssignmentDataModel.getTaskDataModel()), resourceSimulator);
		}
	...
	public long getResourceDownTimeInMillisecondsForTaskOnResource(TaskSimulator taskSimulator,
			ResourceSimulator resourceSimulator, Calendar currentTime, Vector<Assignment> currentLevelAssignments) {
		long durationOfAssignmentInMilliseconds = getSetUpTimeInMillisecondsForTaskOnResource(taskSimulator,
				resourceSimulator, currentTime, currentLevelAssignments)
				+ getOperationTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, currentTime,
		...
		while (durationOfAssignmentInMilliseconds > 0) {
			...
			if (nextNonWorkingDate != null) {
				durationOfAssignmentInMilliseconds = durationOfAssignmentInMilliseconds
						- (nextNonWorkingDate.getTimeInMillis() - currentTime.getTimeInMillis());
				...
				if (durationOfAssignmentInMilliseconds > 0) {
					resourceDownTime = resourceDownTime
							+ (currentTime.getTimeInMillis() - nextNonWorkingDate.getTimeInMillis());
				}
			} else {
	...
	// I have one assumption that the tasks cannot have suitabilities from different
	// workcenters.
	// TWO exceptions exists ... for Mobile Resources that can travel from
	// workcenter to workcenter and for
	// Tools that can be transfered from workcenter to workcenter
	private Vector<TaskSimulator> findSuitableTaskSimulatorsForWorkcenterDataModelFromAvailableTaskSimulators(
			Vector<TaskSimulator> pendingTasksSimulators, WorkcenterDataModel workcenterDataModel) {
		Vector<TaskSimulator> workcenterPendingTasksSimulators = new Vector<TaskSimulator>();
		// NEW IMPLEMENTATION: check only if task belongs to a job released on the
		// current workcenter
		for (TaskSimulator taskSimulator : pendingTasksSimulators) {
			...
			for (JobDataModel jobDataModel : jobDataModelVector) {
				if (jobDataModel.getWorkcenterDataModel() == null
						&& jobDataModel.getTasks().contains(taskSimulator.getTaskDataModel())) {
					// Means that there is not job/workcenter constrain so check incrementally
					Vector<ResourceSimulator> suitabeResourceSimulators = getSuitableResourcesSimulatorForTask(
							taskSimulator);
					for (ResourceSimulator resourceSimulator : suitabeResourceSimulators) {
						if (workcenterDataModel.containsResourceDataModel(resourceSimulator.getResourceDataModel())) {
							...
							for (ResourceDataModel workcenterResourceDataModel : workcenterDataModel
									.getResourceDataModelVector()) {
								if (workcenterResourceDataModel.getEndEffectorsDataModel().getSuportedToolTypes()
										.contains(
												((ToolDataModel) resourceSimulator.getResourceDataModel()).getType())) {
							...
							if (workcenterDataModel.getDockingStationDataModelVector() != null) {
								for (DockingStationDataModel workcenterDockingStationDataModel : workcenterDataModel
										.getDockingStationDataModelVector()) {
									if (workcenterDockingStationDataModel.getSuportedMobileResourceTypes().contains(
											((MobileResourceDataModel) resourceSimulator.getResourceDataModel())
													.getType())) {
					...
					break;
				} else if (jobDataModel.getWorkcenterDataModel() != null
						&& jobDataModel.getWorkcenterDataModel().equals(workcenterDataModel)
						&& jobDataModel.getTasks().contains(taskSimulator.getTaskDataModel())) {
					workcenterPendingTasksSimulators.add(taskSimulator);
					break;
				}
	...
	/**
	 * This method checks if the task is locked or if the task can be performed
	 * uninterrupted. If task is locked then checks the preassigned resource and
	 * then the preassigned time.
	 * 
	 * @param resourceSimulator The resource
	 * @param taskSimulator     The task
	...
	public boolean canResourceFullfilTheTaskUniterrupted(ResourceSimulator resourceSimulator,
			TaskSimulator taskSimulator, Calendar time) {
		// Check for suitability...
		if (!isTaskSuitableForResource(taskSimulator, resourceSimulator))
			return false;

		// Checking the locked constraints of the task
		Vector<AssignmentDataModel> assignmentDataModelVector = prereleasedAssignments
				.getPrereleaseAssignmentDataModelVector();
		for (int i = 0; i < assignmentDataModelVector.size(); i++) {
		...
		for (int i = 0; i < assignmentDataModelVector.size(); i++) {
			...
			if (assignment.getResourceDataModel().equals(resourceSimulator.getResourceDataModel())) {
				...
				if (assignment.getTimeOfDispatch().getTimeInMillis() < time.getTimeInMillis()) {
					...
					// time now.
					if (assignment.getTimeOfDispatch().getTimeInMillis() + assignment.getDurationInMilliseconds() > time
							.getTimeInMillis()) {
					...
					if (time.getTimeInMillis()
							+ getOperationTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, time,
									null)
							+ getSetUpTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, time, null)
							+ getResourceDownTimeInMillisecondsForTaskOnResource(taskSimulator, resourceSimulator, time,
									null) > assignment.getTimeOfDispatch().getTimeInMillis()) {
	...
	public ManualPlanHelperInterface getManualPlanningHelperInterface() {
		...
		manualSimulator.workcenterDataModelVector = (Vector<WorkcenterDataModel>) this.workcenterDataModelVector
				.clone();
		manualSimulator.precedenceConstraintDataModelVector = (Vector<TaskPrecedenceConstraintDataModel>) this.precedenceConstraintDataModelVector
				.clone();
		manualSimulator.taskSuitableResourceDataModelVector = (Vector<TaskSuitableResourceDataModel>) this.taskSuitableResourceDataModelVector
				.clone();
		...
		Vector<AssignmentDataModel> prereleasedAssignmentsForManualPlanning = (Vector<AssignmentDataModel>) prereleasedAssignmentDataModelVector
				.clone();
		prereleasedAssignmentsForManualPlanning
				.addAll((Vector<AssignmentDataModel>) this.assignments.getAssignmentDataModelVector().clone());
		manualSimulator.prereleasedAssignments = new PoolOfPrereleasedAssignmentsSimulator(
				prereleasedAssignmentsForManualPlanning);
		manualSimulator.assignments = new PoolOfAssignmentsSimulator(new Vector<AssignmentDataModel>());

		// And lastly the temp variables ...
		manualSimulator.prereleasedAssignmentDataModelVector = (Vector<AssignmentDataModel>) prereleasedAssignmentsForManualPlanning
				.clone();
	...
	public void setManualTimeNow(Calendar newTimeNow) {
		...
		// Checking for pre released (locked) assignments before the start date of the
		// schedule and dispatch them ...
		// Also if they are finished before the start date inform the corresponding
		// resources.
		Vector<AssignmentDataModel> allPrereleasedAssignments = this.prereleasedAssignments
				.getPrereleaseAssignmentDataModelVector();
		for (int i = 0; i < allPrereleasedAssignments.size(); i++) {
			AssignmentDataModel prereleasedAssignment = allPrereleasedAssignments.get(i);
			if (timeNow.getTimeInMillis() >= allPrereleasedAssignments.get(i).getTimeOfDispatch().getTimeInMillis()) {
				// Since the pre-released assignment must be assigned make a new one with
				// updated set up and operational times and replace the old
				AssignmentDataModel newPrereleasedAssignment = new AssignmentDataModel(
						prereleasedAssignment.getTaskDataModel(), prereleasedAssignment.getResourceDataModel(),
						prereleasedAssignment.getTimeOfDispatch(),
						getSetUpTimeInMillisecondsForTaskOnResource(
	...
	public WorkcenterDataModel locateResource(ResourceDataModel resourceDataModel) {
		for (WorkcenterDataModel workcenterDataModel : workcenterDataModelVector) {
			...
			if (workcenterDataModel.getDockingStationDataModelVector() != null) {
				for (DockingStationDataModel dockingStationDataModel : workcenterDataModel
						.getDockingStationDataModelVector()) {
					if (dockingStationDataModel != null && dockingStationDataModel.getCurrentLoad() != null
							&& dockingStationDataModel.getCurrentLoad().equals(resourceDataModel)) {
	...
	public AbstractDataModel locateTool(ToolDataModel toolDataModel) {
		for (WorkcenterDataModel workcenterDataModel : workcenterDataModelVector) {
			...
			if (workcenterDataModel.getInventoryDataModel() != null
					&& workcenterDataModel.getInventoryDataModel().getTools() != null
			...
			for (ResourceDataModel resourceDataModel : workcenterDataModel.getResourceDataModelVector()) {
				if (resourceDataModel.getEndEffectorsDataModel() != null
						&& resourceDataModel.getEndEffectorsDataModel().getTools() != null
			...
			for (DockingStationDataModel dockingStation : workcenterDataModel.getDockingStationDataModelVector()) {
				if (dockingStation.getCurrentLoad() != null) {
					if (dockingStation.getCurrentLoad().getEndEffectorsDataModel() != null) {
						Vector<ToolDataModel> tools = dockingStation.getCurrentLoad().getEndEffectorsDataModel()
								.getTools();
