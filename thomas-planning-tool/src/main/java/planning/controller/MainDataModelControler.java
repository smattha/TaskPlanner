...
public class MainDataModelControler
		implements ControllerToPlanningInputDataModelInterface, ControllerToPlanningOutputDataModelInterface {
	/**
	 ...
	 * Default constructor
	 * 
	 * @param pidm The Planning Input Data Model
	 * @param podm The Planning Output Data Model
	 */
	...
	public void setResourceAvailabilityDataModelToResourceDataModelName(
			ResourceAvailabilityDataModel newResourceAvailabilityDataModel, ResourceDataModel resource) {
		planningInputDataModel.setResourceAvailabilityDataModelToResourceDataModelName(newResourceAvailabilityDataModel,
				resource);
	}

	public void setSetUpCodeToTaskSuitableResourceDataModel(String newSetUpCode,
			TaskSuitableResourceDataModel taskSuitableResourceDataModel) {
	...
	public void removeJobDataModel(JobDataModel job) {
		...
		Vector<TaskSuitableResourceDataModel> suitabilities = planningInputDataModel
				.getTaskSuitableResourceDataModelVector();
		for (int i = 0; i < suitabilities.size(); i++) {
			TaskSuitableResourceDataModel taskSuitableResourceDataModel = (TaskSuitableResourceDataModel) suitabilities
					.get(i);
		...
		for (int i = 0; i < constraints.size(); i++) {
			TaskPrecedenceConstraintDataModel taskPrecedenceConstraintDataModel = (TaskPrecedenceConstraintDataModel) constraints
					.get(i);
			if (tasksToBeRemoved.indexOf(taskPrecedenceConstraintDataModel.getPreconditionTask()) != -1
					|| tasksToBeRemoved.indexOf(taskPrecedenceConstraintDataModel.getPostconditionTask()) != -1) {
	...
	public void removeTaskDataModel(TaskDataModel task) {
		...
		for (int i = 0; i < constraints.size(); i++) {
			...
			if (task == taskPrecedenceConstraintDataModel.getPreconditionTask()
					|| task == taskPrecedenceConstraintDataModel.getPostconditionTask()) {
				constraintsToBeRemoved.add(taskPrecedenceConstraintDataModel);
			}
		}
		constraints.removeAll(constraintsToBeRemoved);
	...
	public void addTaskSuitableResourceDataModel(TaskSuitableResourceDataModel newSuitable) {
		...
		planningInputDataModel.setTaskSuitableResourceDataModelVector(suitabilities);
		// Updating the task start/due dates of the job
		JobDataModel jobDataModel = planningInputDataModel
				.getJobDataModelForTaskDataModel(newSuitable.getTaskDataModel());
		planningInputDataModel.calculateTaskArrivalAndDueDates(jobDataModel);
	}
	...
	public void setTaskDataModelToTaskSuitableResourceDataModel(TaskDataModel task,
			TaskSuitableResourceDataModel taskSuitableResourceDataModel) {
		throw new UnsupportedOperationException("Function not implented yet");
	}

	public void setResourceDataModelToTaskSuitableResourceDataModel(ResourceDataModel resource,
			TaskSuitableResourceDataModel taskSuitableResourceDataModel) {
		throw new UnsupportedOperationException("Function not implented yet");
	}

	public void setOperationTimeInMillisecondsToTaskSuitableResourceDataModel(long operationTimeInMilliseconds,
			TaskSuitableResourceDataModel taskSuitableResourceDataModel) {
		planningInputDataModel.setOperationTimeInMillisecondsToTaskSuitableResourceDataModel(
				operationTimeInMilliseconds, taskSuitableResourceDataModel);
		// Updating the task start/due dates of the job
		JobDataModel jobDataModel = planningInputDataModel
				.getJobDataModelForTaskDataModel(taskSuitableResourceDataModel.getTaskDataModel());
		planningInputDataModel.calculateTaskArrivalAndDueDates(jobDataModel);
	}

	public void addTaskPrecedenceConstraintDataModel(TaskPrecedenceConstraintDataModel newConstraint) {
		JobDataModel jobDataModel = planningInputDataModel
				.getJobDataModelForTaskDataModel(newConstraint.getPreconditionTask());
		// Checking if tasks are of the same job
		if (jobDataModel == planningInputDataModel
				.getJobDataModelForTaskDataModel(newConstraint.getPostconditionTask())) {
	...
	public void removeTaskPrecedenceConstraintDataModel(TaskPrecedenceConstraintDataModel constraint) {
		...
		// Updating the task start/due dates of the job
		JobDataModel jobDataModel = planningInputDataModel
				.getJobDataModelForTaskDataModel(constraint.getPreconditionTask());
		planningInputDataModel.calculateTaskArrivalAndDueDates(jobDataModel);
	}

	public void setPreconditionTaskToTaskPrecedenceConstraintDataModel(TaskDataModel task,
			TaskPrecedenceConstraintDataModel taskPrecedenceConstraintDataModel) {
		throw new UnsupportedOperationException("Function not implented yet");
	}

	public void setPostconditionTaskToTaskPrecedenceConstraintDataModel(TaskDataModel task,
			TaskPrecedenceConstraintDataModel taskPrecedenceConstraintDataModel) {
		throw new UnsupportedOperationException("Function not implented yet");
	}
		...
		for (int i = 0; i < exceptionPeriods.size(); i++) {
			...
			if (currentTimeInMilliseconds > fromDateInMilliseconds) {
				...
				if (periodInMilliseconds != 0) {
					...
					long timeDifferenceFromTheStartOfThePeriodUntilTheCurrentDateInMilliseconds = currentTimeInMilliseconds
							- ((quotient * periodInMilliseconds) + fromDateInMilliseconds);
					if (timeDifferenceFromTheStartOfThePeriodUntilTheCurrentDateInMilliseconds < (toDateInMilliseconds
							- fromDateInMilliseconds)) {
						return false;
					}
				} else {
					if (fromDate.getTimeInMillis() < currentTime.getTimeInMillis()
							&& currentTime.getTimeInMillis() <= toDate.getTimeInMillis()) {
						return false;
					}
				}
				...
				if (periodInMilliseconds != 0) {
					long quotient = differenceFromDateToCurrentDateInMilliseconds / periodInMilliseconds + 1;// We
																												...
																												// up)
					long timeDifferenceFromTheStartOfThePeriodUntilTheCurrentDateInMilliseconds = currentTimeInMilliseconds
							- (fromDateInMilliseconds - (quotient * periodInMilliseconds));
					if (timeDifferenceFromTheStartOfThePeriodUntilTheCurrentDateInMilliseconds < (toDateInMilliseconds
							- fromDateInMilliseconds)) {
						return false;
					}
	...
	public void setDurationInMillisecondsToAssignmentDataModel(long newDurationInMilliseconds,
			AssignmentDataModel assignment) {
	...
	public boolean isTaskSuitableForResource(TaskDataModel taskDataModel, ResourceDataModel resourceDataModel) {
		Vector<TaskSuitableResourceDataModel> taskSuitableResourceDataModelVector = planningInputDataModel
				.getTaskSuitableResourceDataModelVector();
		for (int i = 0; i < taskSuitableResourceDataModelVector.size(); i++) {
			TaskSuitableResourceDataModel taskSuitableResource = (TaskSuitableResourceDataModel) taskSuitableResourceDataModelVector
					.get(i);
			if (taskDataModel.equals(taskSuitableResource.getTaskDataModel())
					&& resourceDataModel.equals(taskSuitableResource.getResourceDataModel())) {
				return true;
			}
		}
		return false;
	...
	/**
	 * This method checks if a task assignment on a given dispatch time with a given
	 * duration (independent of the assigned resource) fulfills his pre and post
	 * conditions
	 * 
	 * @param taskDataModel
	 * @param currentTime
	 ...
	 * @return
	 */
	public boolean hasTaskPendingPresendenceConstraints(TaskDataModel taskDataModel, Calendar currentTime,
			long durationInMilliseconds) {
		Vector<TaskPrecedenceConstraintDataModel> precedenceConstraintDataModelVector = planningInputDataModel
				.getTaskPrecedenceConstraintDataModelVector();
		for (int i = 0; i < precedenceConstraintDataModelVector.size(); i++) {
			TaskPrecedenceConstraintDataModel constraint = (TaskPrecedenceConstraintDataModel) precedenceConstraintDataModelVector
					.get(i);
			...
			if (constraint.getPostconditionTask().equals(taskDataModel)) {
				...
				if (assignment != null) {
					...
					long finishDateInMilliseconds = assignment.getTimeOfDispatch().getTimeInMillis()
							+ assignment.getDurationInMilliseconds();
					if (finishDateInMilliseconds > currentTime.getTimeInMillis()) {
						...
						// System.out.println("Checking Preconditions :: An assignment has been made
						// checking if the currentTime is greater than the pre codition assignment
						// finish date");
			...
			// Checking the post coditions
				...
				if (postcoditionAssignment != null && postcoditionAssignment.getTimeOfDispatch()
						.getTimeInMillis() < currentTime.getTimeInMillis() + durationInMilliseconds) {
	...
	public boolean canResourceFullfilTheTaskUniterrupted(ResourceDataModel resource, TaskDataModel task,
			Calendar timeOfHypotheticalDispatch) {
		// TODO: Implement lock of tasks in a given dispatch time
		Vector<AssignmentDataModel> resourceAssignments = planningOutputDataModel
				.getAssignmentsForResource(resource.getResourceId());
		long durationOfHypotheticalAssignmnetInMilliseconds = this.getSetUpTimeInMillisecondsForTaskOnResource(task,
				resource, timeOfHypotheticalDispatch)
				+ this.getOperationTimeInMillisecondsForTaskOnResource(task, resource)
				+ this.getResourceDownTimeInMillisecondsForTaskOnResource(task, resource, timeOfHypotheticalDispatch);
		...
		if (resourceAssignmentsSize > 0) {
			...
			for (int i = resourceAssignmentsArray.length; --i >= 0;) {
				...
				for (int j = 0; j < i; j++) {
					if (resourceAssignmentsArray[j].getTimeOfDispatch()
							.getTimeInMillis() > resourceAssignmentsArray[j + 1].getTimeOfDispatch()
									.getTimeInMillis()) {
		...
		if (resourceAssignmentsArray != null) {
			for (int i = 0; i < resourceAssignmentsArray.length; i++) {
				...
				long assignmentFinishTimeInMilliseconds = assignment.getTimeOfDispatch().getTimeInMillis()
						+ +getOperationTimeInMillisecondsForTaskOnResource(assignment.getTaskDataModel(),
								assignment.getResourceDataModel())
						+ getSetUpTimeInMillisecondsForTaskOnResource(assignment.getTaskDataModel(),
								assignment.getResourceDataModel(), assignment.getTimeOfDispatch())
						+ getResourceDownTimeInMillisecondsForTaskOnResource(assignment.getTaskDataModel(),
								assignment.getResourceDataModel(), assignment.getTimeOfDispatch());
				if (assignmentFinishTimeInMilliseconds <= timeOfHypotheticalDispatch.getTimeInMillis()) {
					...
					if ((timeOfHypotheticalDispatch.getTimeInMillis()
							+ durationOfHypotheticalAssignmnetInMilliseconds) <= nextAssignment.getTimeOfDispatch()
									.getTimeInMillis())
						return true;
					// }
				} else {
					...
					if (timeOfHypotheticalDispatch.getTimeInMillis()
							+ durationOfHypotheticalAssignmnetInMilliseconds <= assignment.getTimeOfDispatch()
									.getTimeInMillis()) {
						if (i == 0) {
						...
						if (timeOfHypotheticalDispatch
								.getTimeInMillis() >= previousAssignment.getTimeOfDispatch().getTimeInMillis()
										+ previousAssignment.getDurationInMilliseconds())
							return true;
						// }
					}
				}
	...
	public long getOperationTimeInMillisecondsForTaskOnResource(TaskDataModel taskDataModel,
		...
		for (int i = 0; i < taskSuitableResourceDataModelVector.size(); i++) {
			...
			if (taskSuitableResource.getTaskDataModel().equals(taskDataModel)
					&& taskSuitableResource.getResourceDataModel().equals(resourceDataModel)) {
	...
	public String getSetUpCodeForTaskOnResource(TaskDataModel task, ResourceDataModel resource) {
		...
		for (int i = 0; i < taskSuitableResourceDataModelVector.size(); i++) {
			TaskSuitableResourceDataModel taskSuitableResource = taskSuitableResourceDataModelVector.get(i);
			if (taskSuitableResource.getTaskDataModel().equals(task)
					&& taskSuitableResource.getResourceDataModel().equals(resource)) {
	...
	public long getSetUpTimeInMillisecondsForTaskOnResource(TaskDataModel task, ResourceDataModel resource,
		...
		Vector<AssignmentDataModel> assignmentDataModelVector = planningOutputDataModel
				.getAssignmentsForResource(resource.getResourceId());
		for (int i = 0; i < assignmentDataModelVector.size(); i++) {
			...
			long currentAssignmentFinishTimeInMilliseconds = assignmentDataModel.getTimeOfDispatch().getTimeInMillis()
					+ assignmentDataModel.getDurationInMilliseconds();
			if (currentAssignmentFinishTimeInMilliseconds <= currentTime.getTimeInMillis()) {
				if (assignmentDataModel.getResourceDataModel().getResourceId().equals(resource.getResourceId())
						&& latestAssignmentDataModel == null) {
				...
				} else if (assignmentDataModel.getResourceDataModel().getResourceId()
	...
	public long getResourceDownTimeInMillisecondsForTaskOnResource(TaskDataModel task, ResourceDataModel resource,
			Calendar timeNow) {
		long durationOfAssignmentInMilliseconds = getSetUpTimeInMillisecondsForTaskOnResource(task, resource, timeNow)
				+ getOperationTimeInMillisecondsForTaskOnResource(task, resource);
		...
		while (durationOfAssignmentInMilliseconds > 0) {
			...
			if (nextNonWorkingDate != null) {
				durationOfAssignmentInMilliseconds = durationOfAssignmentInMilliseconds
						- (nextNonWorkingDate.getTimeInMillis() - timeNow.getTimeInMillis());
				...
				if (durationOfAssignmentInMilliseconds > 0) {
					resourceDownTime = resourceDownTime
							+ (timeNow.getTimeInMillis() - nextNonWorkingDate.getTimeInMillis());
				}
			} else {
	...
	public void reschedule(Calendar startDate, Vector<AssignmentDataModel> lockedAssignmnets) {
		PlanSimulator simulator = new PlanSimulator(this.planningInputDataModel.getWorkcenterDataModelVector(),
				this.planningInputDataModel.getJobDataModelVector(), lockedAssignmnets,
				this.planningInputDataModel.getTaskPrecedenceConstraintDataModelVector(),
				this.planningInputDataModel.getTaskSuitableResourceDataModelVector(), startDate);
	...
	public void lockRecursive(AssignmentDataModel assignmentDataModel) {
		...
		Vector<TaskPrecedenceConstraintDataModel> constraints = this.planningInputDataModel
				.getTaskPrecedenceConstraintDataModelVector();
		for (int i = 0; i < constraints.size(); i++) {
			...
			if (constraint.getPostconditionTask().equals(assignmentDataModel.getTaskDataModel())) {
				AssignmentDataModel preconditionAssignment = this.planningOutputDataModel
						.getAssignmentForTask(constraint.getPreconditionTask());
	...
	public void unlockRecursive(AssignmentDataModel assignmentDataModel) {
		...
		for (int i = 0; i < constraints.size(); i++) {
			...
			if (constraint.getPreconditionTask().equals(assignmentDataModel.getTaskDataModel())) {
				AssignmentDataModel postconditionAssignment = this.planningOutputDataModel
						.getAssignmentForTask(constraint.getPostconditionTask());
	...
	private Calendar getNextNonWorkingDate(ResourceDataModel resource, Calendar date) {
		if (isResourceDown(resource, date)) {
			...
			Calendar nextNonWorkingDateDueToForcedWorkingPeriods = getNextNonWorkingDateFromForcedWorkingDates(resource,
					date);
			if (nextNonWorkingDateDueToForcedWorkingPeriods != null && (nextNonWorkingDateDueToForcedWorkingPeriods
					.getTimeInMillis() < nextWorkingDateDueToNonWorkingPeriods.getTimeInMillis())) {
				return nextNonWorkingDateDueToForcedWorkingPeriods;
			}
			// else
			{
				Calendar nextNonWorkingDateDueToNonWorkingPeriods = getNextNonWorkingDateFromNonWorkingDates(resource,
						date);
				Calendar nextNonWorkingDateDueToForcedWorkingPeriods2 = getNextNonWorkingDateFromForcedWorkingDates(
						resource, nextNonWorkingDateDueToNonWorkingPeriods);
		...
		// else
		{
			if (isDateInsideTheResoureNonWorkingPeriods(date, resource)) {
				...
				Calendar nextNonWorkingDateDueToForcedWorkingPeriods = getNextNonWorkingDateFromForcedWorkingDates(
						resource, date);
				Calendar nextWorkingDateDueToNonWorkingPeriods = getNextWorkingDateFromNonWorkingDates(resource, date);
				if (nextWorkingDateDueToNonWorkingPeriods
						.getTimeInMillis() > nextNonWorkingDateDueToForcedWorkingPeriods.getTimeInMillis()) {
					return nextNonWorkingDateDueToForcedWorkingPeriods;
				}
				// else
				{
					Calendar nextNonWorkingDateDueToNonWorkingPeriods = getNextNonWorkingDateFromNonWorkingDates(
							resource, date);
					Calendar nextNonWorkingDateDueToForcedWorkingPeriods2 = getNextNonWorkingDateFromForcedWorkingDates(
							resource, nextNonWorkingDateDueToNonWorkingPeriods);
					return nextNonWorkingDateDueToForcedWorkingPeriods2;
	...
	private Calendar getNextNonWorkingDateFromNonWorkingDates(ResourceDataModel resource, Calendar date) {
		...
		while (isDateInsideTheResoureNonWorkingPeriods(copyOfDateToCheck, resource)) {
			...
			for (int i = 0; i < resourceNonWorkingPeriods.size(); i++) {
				...
				long durationInMilliseconds = timePeriod.getToDate().getTimeInMillis()
						- startPeriodDate.getTimeInMillis();
				if (copyOfDateToCheck.getTimeInMillis() > startPeriodDate.getTimeInMillis()) {
					if (periodInMilliseconds != 0) {
						long periodTimes = (copyOfDateToCheck.getTimeInMillis() - startPeriodDate.getTimeInMillis())
								/ periodInMilliseconds;
						if (startPeriodDate.getTimeInMillis() + (periodTimes * periodInMilliseconds)
								+ durationInMilliseconds > copyOfDateToCheck.getTimeInMillis()) {
							copyOfDateToCheck.setTimeInMillis(startPeriodDate.getTimeInMillis()
									+ (periodTimes * periodInMilliseconds) + durationInMilliseconds);
						}
					} else {
					...
					if (periodInMilliseconds != 0) {
						// TODO: check if start period copyOfDateToCheck is after the copyOfDateToCheck
						// assuming not and continue...
						throw new RuntimeException(
								"Start period copyOfDateToCheck is after the copyOfDateToCheck assuming not implemented yet");
					}
					// else
					{
						if (timePeriod.getToDate().getTimeInMillis() > copyOfDateToCheck.getTimeInMillis()) {
			...
			// TODO : Find a condition in order to end the loop if the resource is
			// completely DOWN.
			...
			if (copyOfDateToCheck.getTimeInMillis() > startPeriodDate.getTimeInMillis()) {
				...
				if (periodInMilliseconds != 0) {
					...
					nextNonWorkingDateInMilliseconds = startPeriodDate.getTimeInMillis()
							+ periodTimes * periodInMilliseconds;
	...
	private Calendar getNextNonWorkingDateFromForcedWorkingDates(ResourceDataModel resource, Calendar date) {
		...
		for (int i = 0; i < resourceForcedWorkingPeriods.size(); i++) {
			...
			if (date.getTimeInMillis() > startPeriodDate.getTimeInMillis()) {
				long nextNonWorkingDateInMilliseconds = 0;
				if (periodInMilliseconds != 0) {
					long periodTimes = (date.getTimeInMillis() - startPeriodDate.getTimeInMillis())
							/ periodInMilliseconds;
					if (date.getTimeInMillis() <= startPeriodDate.getTimeInMillis() + periodTimes * periodInMilliseconds
				...
				if (periodInMilliseconds != 0) {
					// TODO: check if start period date is after the date assuming not and
					// continue...
	...
	protected Calendar getNextWorkingDate(ResourceDataModel resource, Calendar date) {
		if (isResourceDown(resource, date)) {
			Calendar nextWorkingDateDueToNonWorkingPeriods = getNextWorkingDateFromNonWorkingDates(resource, date);
			Calendar nextWorkingDateDueToForcedWorkingPeriods = getNextWorkingDateFromForcedWorkingDates(resource,
					date);
			...
			} else if (nextWorkingDateDueToNonWorkingPeriods
					.getTimeInMillis() < nextWorkingDateDueToForcedWorkingPeriods.getTimeInMillis()) {
				return nextWorkingDateDueToNonWorkingPeriods;
			} else {
		...
		for (int i = 0; i < resourceNonWorkingPeriods.size(); i++) {
			...
			if (date.getTimeInMillis() > startPeriodDate.getTimeInMillis()) {
				...
				if (periodInMilliseconds != 0) {
					long periodTimes = (date.getTimeInMillis() - startPeriodDate.getTimeInMillis())
							/ periodInMilliseconds;
					nextWorkingDateInMilliseconds = startPeriodDate.getTimeInMillis()
							+ periodTimes * periodInMilliseconds + durationInMilliseconds;
		...
		for (int i = 0; i < resourceNonWorkingPeriods.size(); i++) {
			...
			if (date.getTimeInMillis() > startPeriodDate.getTimeInMillis()) {
				if (periodInMilliseconds != 0) {
					long periodTimes = (date.getTimeInMillis() - startPeriodDate.getTimeInMillis())
							/ periodInMilliseconds;
					if (startPeriodDate.getTimeInMillis() + (periodInMilliseconds * periodTimes) <= date
							.getTimeInMillis()
							&& date.getTimeInMillis() < startPeriodDate.getTimeInMillis()
									+ (periodInMilliseconds * periodTimes) + durationInMilliseconds) {
						return true;
					}
				} else {
					if (startPeriodDate.getTimeInMillis() <= date.getTimeInMillis()
							&& date.getTimeInMillis() < timePeriod.getToDate().getTimeInMillis()) {
						return true;
					}
				}
			} else {
