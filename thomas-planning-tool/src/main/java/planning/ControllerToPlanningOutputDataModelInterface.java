...
public interface ControllerToPlanningOutputDataModelInterface {
	...
	public boolean hasTaskPendingPresendenceConstraints(TaskDataModel task, Calendar timeNow,
			long durationInMilliseconds);// Checks for constrains

	public boolean canResourceFullfilTheTaskUniterrupted(ResourceDataModel resource, TaskDataModel task,
			Calendar timeOfHypotheticalDispatch);// Also checks for locked tasks

	public long getOperationTimeInMillisecondsForTaskOnResource(TaskDataModel task, ResourceDataModel resource);// Returns
																												// the
																												// pure
																												// operation
																												// time
	...
	public String getSetUpCodeForTaskOnResource(TaskDataModel task, ResourceDataModel resource);// Returns the set up
																								// code of this
																								// potential assignment

	public long getSetUpTimeInMillisecondsForTaskOnResource(TaskDataModel task, ResourceDataModel resource,
			Calendar currentTime);// Returns the set up time of this assignment

	@Deprecated
	public long getSetUpTimeInMillisecondsFromSetUpCodeToSetUpCodeOnResource(String fromSetUpCode, String toSetUpCode,
			ResourceDataModel resource);// Returns the set up time of this assignment

	public long getResourceDownTimeInMillisecondsForTaskOnResource(TaskDataModel task, ResourceDataModel resource,
			Calendar timeNow);// Returns the down time of the resource during the period of execution of this
								// assignment (operation time + set up time)
