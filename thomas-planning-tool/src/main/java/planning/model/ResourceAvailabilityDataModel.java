...
public class ResourceAvailabilityDataModel extends AbstractDataModel {
	...
	public ResourceAvailabilityDataModel(Vector<TimePeriodDataModel> nonWorkingPeriodDataModelVector,
			Vector<TimePeriodDataModel> forcedWorkingPeriodDataModelVector, String id) {
	...
	// TODO : this constructor should be deprecated so the software uses the other
	// constructor
	@Deprecated
	public ResourceAvailabilityDataModel(Vector<TimePeriodDataModel> nonWorkingPeriodDataModelVector,
			Vector<TimePeriodDataModel> forcedWorkingPeriodDataModelVector) {
		this.nonWorkingPeriods = nonWorkingPeriodDataModelVector;
		this.exceptionsNonWorkingPeriods = forcedWorkingPeriodDataModelVector;
	...
	public Node toXMLNode(Document document) {
		...
		for (int i = 0; i < this.nonWorkingPeriods.size(); i++) {
			nonWorkingPeriodsElement
					.appendChild(((TimePeriodDataModel) this.nonWorkingPeriods.get(i)).toXMLNode(document));
		}
		...
		for (int i = 0; i < this.exceptionsNonWorkingPeriods.size(); i++) {
			forcedWorkingPeriodsElement
					.appendChild(((TimePeriodDataModel) this.exceptionsNonWorkingPeriods.get(i)).toXMLNode(document));
		}
