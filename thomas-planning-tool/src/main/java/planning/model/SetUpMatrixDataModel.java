...
public class SetUpMatrixDataModel extends AbstractDataModel {
	...
	public Node toXMLNode(Document document) {
		...
		for (int i = 0; i < setUpCodesAndTimes.size(); i++) {
			...
			timeInSecondsElement.appendChild(
					document.createTextNode("" + (((Long) details.get(TIME_IN_MILLISECONDS)).longValue() / 1000)));
	...
	public long getSetUpTimeInMilliseconds(String fromSetUpCode, String toSetUpCode) {
		...
		// throw new
		// RuntimeException("Set up for codes FROM : "+fromSetUpCode+" TO :
		// "+toSetUpCode+" cannot be found");
		return 0;
	}
