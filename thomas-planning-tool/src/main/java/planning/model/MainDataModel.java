...
public class MainDataModel extends AbstractDataModel {
	...
	public MainDataModel(AbstractInputSource inputSource) {
		...
		// Todo :
		// 1) check persistency ammong data (the tasks\resources that exist in the
		// output model to allready exist in the input model
		try {
		...
		try {
			...
			XMLToPlanningOutputModel xmlToPlanningOutputModel = new XMLToPlanningOutputModel(planningOutputXMLDoument,
					planningInputDataModel);
...
//            ex.printStackTrace();
	...
	public MainDataModel(PlanningInputDataModel planningInputDataModel,
			PlanningOutputDataModel planningOutputDataModel) {
