...
public class PlanningInputDataModel extends AbstractDataModel implements ViewToPlanningInputDataModelInterface {
	...
	public PlanningInputDataModel(Vector<ResourceDataModel> resources, Vector<TaskDataModel> tasks,
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints,
			Vector<TaskSuitableResourceDataModel> tasksSuitableResources, Vector<WorkcenterDataModel> workcenters,
			Vector<JobDataModel> jobs, Vector<ToolTypeDataModel> toolTypes, Vector<ToolDataModel> tools,
			Vector<MobileResourceTypeDataModel> mobileResourceTypes, Vector<MobileResourceDataModel> mobileResources) {
	...
	public void setTaskPrecedenceConstraintsDataModelVector(
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints) {
		// Todo :
		// 1) implementation and check of pre initialized values
		// i.e. : if the attribute is null (not initialized) then set this
	...
	public void calculateTaskArrivalAndDueDates(JobDataModel jobDataModel) {
		...
		for (int j = 0; j < taskDataModelVector.size(); j++) {
			...
			Vector<TaskPrecedenceConstraintDataModel> currentTaskDataModelPrecedenceConstraints = getPrecedenceConstraintsForTaskDataModel(
					currentTaskDataModel, precedenceConstraints);
																		...
																		// job
			{
				...
				System.out.println("task : " + currentTaskDataModel.getTaskName() + " initialized with arrival : "
						+ currentTaskDataModel.getTaskArrivalDate().getTime() + " , due : "
						+ currentTaskDataModel.getTaskDueDate().getTime());
					...
					// job
			{
				...
				for (int k = 0; k < currentTaskDataModelPrecedenceConstraints.size(); k++) {
					TaskPrecedenceConstraintDataModel currentTaskDataModelPrecedenceConstraint = currentTaskDataModelPrecedenceConstraints
							.get(k);
					if (jobDataModel.contains(currentTaskDataModelPrecedenceConstraint.getPreconditionTask())
							&& jobDataModel.contains(currentTaskDataModelPrecedenceConstraint.getPostconditionTask())) {
		...
		for (int j = 0; j < allTaskNodes.size(); j++) {
			...
			if (getPreconditionsConstraintsForTaskDataModelInsideTheSameJob(jobDataModel, task, precedenceConstraints)
					.size() == 0) {
				// Root Node found
				TaskNode rootNode = taskNode;

				// Building paths
				Vector<Vector<TaskNode>> rootPaths = getAllPathsForTheRoot(jobDataModel, rootNode,
						precedenceConstraints, allTaskNodes);
				...
				for (int k = 0; k < rootPaths.size(); k++) {
					...
					for (int l = 0; l < singlePath.size(); l++) {
						...
						if (suitableResourcesSize != 0)
							meanOperationalTimeInMilliseconds = sumOperationalTimeInMilliseconds
									/ suitableResourcesSize;
						else
					...
					long jobDurationInMilliseconds = jobDataModel.getDueDate().getTimeInMillis()
							- jobDataModel.getArrivalDate().getTimeInMillis();
					for (int l = 0; l < singlePath.size(); l++) {
						// Getting the Node
						TaskNode pathTaskNode = singlePath.get(l);
						// Calculating the weight
						double weight = (double) pathTaskNode.getMeanOperationalTimeInMilliseconds()
								/ (double) pathOperationTimeInMilliseconds;
						...
						if (previousCalculatedDueDate == null) {
							...
							// job arrival date
							long calendarDueDateInMilliseconds = jobDataModel.getArrivalDate().getTimeInMillis()
							...
							// due date
							long calendarDueDateInMilliseconds = previousCalculatedDueDate.getTimeInMillis()
									+ taskDurationInMilliseconds;
							Calendar taskDueDate = Calendar.getInstance();
							taskDueDate.setTimeInMillis(calendarDueDateInMilliseconds);
							// Setting the just calculated due date to the task
		...
		for (int j = 0; j < allTaskNodes.size(); j++) {
			...
			System.out.println("task : " + task.getTaskName() + " initialized with arrival : "
					+ task.getTaskArrivalDate().getTime() + " , due : " + task.getTaskDueDate().getTime());
		}
	}

	private Vector<Vector<TaskNode>> getAllPathsForTheRoot(JobDataModel job, TaskNode root,
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints, TaskNodeVector allTaskNodes) {
		Vector<Vector<TaskNode>> paths = new Vector<Vector<TaskNode>>();

		Vector<TaskPrecedenceConstraintDataModel> nextNodesConstraints = getPostconditionsConstraintsForTaskDataModelInsideTheSameJob(
				job, root.getTaskDataModel(), precedenceConstraints);
		if (nextNodesConstraints.size() == 0) {
			...
			for (int i = 0; i < nextNodesConstraints.size(); i++) {
				TaskNode taskNode = allTaskNodes
						.getTaskNodeForTaskDataModel(nextNodesConstraints.get(i).getPostconditionTask());
				Vector<Vector<TaskNode>> subPaths = getAllPathsForTheRoot(job, taskNode, precedenceConstraints,
						allTaskNodes);
	...
	private Vector<TaskPrecedenceConstraintDataModel> getPrecedenceConstraintsForTaskDataModel(
			TaskDataModel taskDataModel, Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints) {
		...
		for (int i = 0; i < precedenceConstraints.size(); i++) {
			...
			if (constraint.getPostconditionTask().equals(taskDataModel)
					|| constraint.getPreconditionTask().equals(taskDataModel)) {
	...
	// Helper function for the initialization of task arrival/due dates
	private Vector<TaskPrecedenceConstraintDataModel> getPostconditionsConstraintsForTaskDataModelInsideTheSameJob(
			JobDataModel jobDataModel, TaskDataModel taskDataModel,
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints) {
		Vector<TaskPrecedenceConstraintDataModel> precedenceConstraintsForTaskDataModel = new Vector<TaskPrecedenceConstraintDataModel>();
		for (int i = 0; i < precedenceConstraints.size(); i++) {
			TaskPrecedenceConstraintDataModel constraint = precedenceConstraints.get(i);
			// Check if task has preconditions
			if (constraint.getPreconditionTask().equals(taskDataModel)
					&& jobDataModel.contains(constraint.getPostconditionTask())) {
				precedenceConstraintsForTaskDataModel.add(constraint);
			}
		}
		return precedenceConstraintsForTaskDataModel;
	...
	private Vector<TaskPrecedenceConstraintDataModel> getPreconditionsConstraintsForTaskDataModelInsideTheSameJob(
			JobDataModel jobDataModel, TaskDataModel taskDataModel,
			Vector<TaskPrecedenceConstraintDataModel> precedenceConstraints) {
		Vector<TaskPrecedenceConstraintDataModel> precedenceConstraintsForTaskDataModel = new Vector<TaskPrecedenceConstraintDataModel>();
		for (int i = 0; i < precedenceConstraints.size(); i++) {
			...
			// Check if task has preconditions
			if (constraint.getPostconditionTask().equals(taskDataModel)
					&& jobDataModel.contains(constraint.getPreconditionTask())) {
				precedenceConstraintsForTaskDataModel.add(constraint);
			}
		}
		return precedenceConstraintsForTaskDataModel;
