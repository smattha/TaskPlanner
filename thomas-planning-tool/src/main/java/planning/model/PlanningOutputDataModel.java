...
public class PlanningOutputDataModel extends AbstractDataModel implements ViewToPlanningOutputDataModelInterface {
	...
	// TODO : Check if and when recalculation should be performed upon
	// planningInputDataModel changes
	private void recalculatePerformanceIndicators() {
		if (assignments != null && assignments.size() > 0) {
			...
			for (int i = 0; i < assignmentsCount; i++) {
				...
				long assignmentCompletion = assignment.getTimeOfDispatch().getTimeInMillis()
						+ assignment.getDurationInMilliseconds();
				// Calculate the assignment flow time and add it to the sum
				long assignmentFlowTimeInMilliseconds = assignmentCompletion
						- task.getTaskArrivalDate().getTimeInMillis();
