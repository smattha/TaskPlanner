        ...
        for(int i = 0; i<assignmentsNodes.getLength(); i++)
        {
            ...
            for(int j = 0; j<assignmentDetails.getLength(); j++)
            {
                if(!assignmentDetails.item(j).getNodeName().equals("#text"))
                {
                    ...
                    {
                        timeOfDispatch = XMLUtil.getCalendarDateFromXMLNodeList(assignmentDetails.item(j).getChildNodes());
                    }
            ...
            try
            {
                ...
                {
                    assignment = new AssignmentDataModel(task, resource, timeOfDispatch, durationInMilliseconds, true, assignmentProperties);
                }
                else
                {
                    assignment = new AssignmentDataModel(task, resource, timeOfDispatch, durationInMilliseconds, false, assignmentProperties);
                }
