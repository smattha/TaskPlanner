...
public class XMLToPlanningInputModel {
	...
	public PlanningInputDataModel getPlanningInputDataModel() {
		if (inputModel == null) {
			...
			// Done with the model data, perform some initialisation procedure in order to
	...
	private Vector<MobileResourceDataModel> getMobileResources() {
		...
		for (int i = 0; i < mobileResourceNodes.getLength(); i++) {
					...
					if (resourceDetailsNodes.item(j).getNodeName().equals("DESCRIPTION")) {
						...
						for (int k = 0; k < resourceAvailabilityDetailsNodes.getLength(); k++) {
							if (!resourceAvailabilityDetailsNodes.item(k).getNodeName().equals("#text")) {
								if (resourceAvailabilityDetailsNodes.item(k).getNodeName()
										.equals("NON_WORKING_PERIODS")) {
									NodeList nonWorkingPeriodsNodeList = resourceAvailabilityDetailsNodes.item(k)
											.getChildNodes();
									...
									for (int l = 0; l < nonWorkingPeriodsNodeList.getLength(); l++) {
										if (!nonWorkingPeriodsNodeList.item(l).getNodeName().equals("#text")) {
											if (nonWorkingPeriodsNodeList.item(l).getNodeName().equals("PERIOD")) {
												...
												NodeList nonWorkingPeriodDetailsNodeList = nonWorkingPeriodsNodeList
														.item(l).getChildNodes();
												for (int m = 0; m < nonWorkingPeriodDetailsNodeList.getLength(); m++) {
													if (!nonWorkingPeriodDetailsNodeList.item(m).getNodeName()
															.equals("#text")) {
														if (nonWorkingPeriodDetailsNodeList.item(m).getNodeName()
																.equals("FROM_DATE")) {
															fromDate = XMLUtil.getCalendarDateFromXMLNodeList(
																	nonWorkingPeriodDetailsNodeList.item(m)
																			.getChildNodes());
															// System.out.println("FROM XML fromDate :
															// "+fromDate.getTime());
														} else if (nonWorkingPeriodDetailsNodeList.item(m).getNodeName()
																.equals("TO_DATE")) {
															toDate = XMLUtil.getCalendarDateFromXMLNodeList(
																	nonWorkingPeriodDetailsNodeList.item(m)
																			.getChildNodes());
															// System.out.println("FROM XML toDate :
															// "+toDate.getTime());
														} else if (nonWorkingPeriodDetailsNodeList.item(m).getNodeName()
																.equals("REOCCURANCE")) {
															// Getting the children elements (it should only be one
															// exept if it defines time)
															NodeList reoccuranceChildsNideList = nonWorkingPeriodDetailsNodeList
																	.item(m).getChildNodes();
															// Looping among them. It should be one exept if it defines
															// time, that's wh the loop is essential
															for (int n = 0; n < reoccuranceChildsNideList
																	.getLength(); n++) {
																if (!reoccuranceChildsNideList.item(n).getNodeName()
																		.equals("#text")) {
																	if (reoccuranceChildsNideList.item(n).getNodeName()
																			.equals("YEARS")) {
																		...
																		String ammountOfRecurrenceString = reoccuranceChildsNideList
																				.item(n).getLastChild().getNodeValue();
																		try {
																			ammountOfRecurrence = Integer.parseInt(
																					ammountOfRecurrenceString);
																		} catch (Exception ex) {
																			// Do not print anything this exception
																			// occured to wrong xml values. Continue
																			// with defaults.
																		}
																	} else if (reoccuranceChildsNideList.item(n)
																			.getNodeName().equals("MONTHS")) {
																		// Setting the timePeriod
												...
												if (TimePeriodDataModel.PERIOD_IN_TIME.equals(recurrencePeriod)) {
													nonWorkingPeriod = new TimePeriodDataModel(fromDate, toDate, hours,
															minutes, seconds);
												} else {
													nonWorkingPeriod = new TimePeriodDataModel(fromDate, toDate,
															recurrencePeriod, ammountOfRecurrence);
												}
								...
								} else if (resourceAvailabilityDetailsNodes.item(k).getNodeName()
										.equals("FORCED_WORKING_PERIODS")) {
									NodeList forcedWorkingPeriodsNodeList = resourceAvailabilityDetailsNodes.item(k)
											.getChildNodes();
									// Looping among the periods (non_working_periods element child nodes)
									for (int l = 0; l < forcedWorkingPeriodsNodeList.getLength(); l++) {
										if (!forcedWorkingPeriodsNodeList.item(l).getNodeName().equals("#text")) {
											if (forcedWorkingPeriodsNodeList.item(l).getNodeName().equals("PERIOD")) {
												...
												// Looping among the period's data
												NodeList forcedWorkingPeriodDetailsNodeList = forcedWorkingPeriodsNodeList
														.item(l).getChildNodes();
												for (int m = 0; m < forcedWorkingPeriodDetailsNodeList
														.getLength(); m++) {
													if (!forcedWorkingPeriodDetailsNodeList.item(m).getNodeName()
															.equals("#text")) {
														if (forcedWorkingPeriodDetailsNodeList.item(m).getNodeName()
																.equals("FROM_DATE")) {
															fromDate = XMLUtil.getCalendarDateFromXMLNodeList(
																	forcedWorkingPeriodDetailsNodeList.item(m)
																			.getChildNodes());
															// System.out.println("FROM XML fromDate :
															// "+fromDate.getTime());
														} else if (forcedWorkingPeriodDetailsNodeList.item(m)
																.getNodeName().equals("TO_DATE")) {
															toDate = XMLUtil.getCalendarDateFromXMLNodeList(
																	forcedWorkingPeriodDetailsNodeList.item(m)
																			.getChildNodes());
															// System.out.println("FROM XML toDate :
															// "+toDate.getTime());
																...
																.getNodeName().equals("REOCCURANCE")) {
															// Getting the children elements (it should only be one
															// exept if it defines time)
															NodeList reoccuranceChildsNideList = forcedWorkingPeriodDetailsNodeList
																	.item(m).getChildNodes();
															// Looping among them. It should be one exept if it defines
															// time, that's wh the loop is essential
															for (int n = 0; n < reoccuranceChildsNideList
												...
												// Puting the values alltogether constracting a period object
												if (TimePeriodDataModel.PERIOD_IN_TIME.equals(recurrencePeriod)) {
													forcedWorkingPeriod = new TimePeriodDataModel(fromDate, toDate,
			...
			mobileResourceAvailability = new ResourceAvailabilityDataModel(mobileResourceNonWorkingPeriods,
					mobileResourceForcedWorkingPeriods);

			MobileResourceDataModel mobileResource = new MobileResourceDataModel(mobileResourceId, mobileResourceName,
					mobileResourceDescription, mobileResourceAvailability, mobileResourceSetUpMatrixDataModel,
					mobileResourceEndEffectorDataModel, mobileResourceType, mobileResourceProperties);
				...
				if (!mobileResourceTypeDetailsNodes.item(j).getNodeName().equals("#text")) {
					if (mobileResourceTypeDetailsNodes.item(j).getNodeName().equals("TYPE_NAME")) {
						if (mobileResourceTypeDetailsNodes.item(j).getLastChild() != null)
							mobileResourceTypeType = mobileResourceTypeDetailsNodes.item(j).getLastChild()
									.getNodeValue();
					}
				}
			}
			MobileResourceTypeDataModel mobileResourceType = new MobileResourceTypeDataModel(mobileResourceTypeId,
					mobileResourceTypeType);
	...
	private Vector<ToolDataModel> getTools() {
		...
		for (int i = 0; i < toolNodes.getLength(); i++) {
			...
			ToolDataModel mobileResource = new ToolDataModel(toolId, toolName, toolDescription, toolAvailability,
					toolSetUpMatrixDataModel, toolType, toolProperties);
			...
			for (int j = 0; j < constraintDetails.getLength(); j++) {
				if (!constraintDetails.item(j).getNodeName().equals("#text")) {
					if (constraintDetails.item(j).getNodeName().equals("PRECONDITION_TASK_REFERENCE")) {
						...
						NamedNodeMap attributes = constraintDetails.item(j).getAttributes();
						postconditiontask = inputModel
								.getTaskDataModel(attributes.getNamedItem("refid").getNodeValue());
					} else if (constraintDetails.item(j).getNodeName().equals("NEXT_TASK_IN_CHAIN")) {
						if (constraintDetails.item(j).getLastChild() != null)
							isConstraintTypeNextTaskInChain = Boolean
									.parseBoolean(constraintDetails.item(j).getLastChild().getNodeValue());
					} else if (constraintDetails.item(j).getNodeName().equals("RESOURCE_UNAVAILABLE_UNTIL_NEXT_TASK")) {
						if (constraintDetails.item(j).getLastChild() != null)
							isTasksPreemptionAllowed = Boolean
									.parseBoolean(constraintDetails.item(j).getLastChild().getNodeValue());
					}
				}
			}
			TaskPrecedenceConstraintDataModel constraint = new TaskPrecedenceConstraintDataModel(preconditiontask,
					postconditiontask, isConstraintTypeNextTaskInChain, isTasksPreemptionAllowed);
	...
	private Vector<TaskSuitableResourceDataModel> getTasksSuitableResources() {
		...
		for (int i = 0; i < taskSuitableResourcesNodes.getLength(); i++) {
			...
			for (int j = 0; j < taskSuitableResourceDetails.getLength(); j++) {
				if (!taskSuitableResourceDetails.item(j).getNodeName().equals("#text")) {
					if (taskSuitableResourceDetails.item(j).getNodeName().equals("TASK_REFERENCE")) {
						...
						resource = inputModel
								.getMobileResourceDataModel(attributes.getNamedItem("refid").getNodeValue());
					} else if (taskSuitableResourceDetails.item(j).getNodeName()
							.equals("OPERATION_TIME_PER_BATCH_IN_SECONDS")) {
			...
			TaskSuitableResourceDataModel taskSuitableResource = new TaskSuitableResourceDataModel(task, resource,
					(long) (operationTime * 1000d), setUpCode, properties);
	...
	private Vector<WorkcenterDataModel> getWorkcenters() {
		...
		for (int i = 0; i < workcenterNodes.getLength(); i++) {
			...
			for (int j = 0; j < workcenterDetails.getLength(); j++) {
				if (!workcenterDetails.item(j).getNodeName().equals("#text")) {
					if (workcenterDetails.item(j).getNodeName().equals("WORKCENTER_RESOURCE_REFERENCE")) {
						...
						ResourceDataModel resource = inputModel
								.getResourceDataModel(attributes.getNamedItem("refid").getNodeValue());
			...
			WorkcenterDataModel workcenter = new WorkcenterDataModel(workcenterId, name, description, algorithm,
					resources, inventoryDataModel, dockingStationDataModels);
					...
					for (int j = 0; j < supportedMobileResourceTypes.getLength(); j++) {
						...
						if (!supportedMobileResourceType.getNodeName().equals("#text")) {
							if (supportedMobileResourceType.getNodeName().equals("MOBILE_RESOURCE_TYPE_REFERENCE")) {
								...
								if (referenceAttributes.getNamedItem("refid") != null) {
									mobileResourceTypeReference = referenceAttributes.getNamedItem("refid")
											.getNodeValue();
								}
								MobileResourceTypeDataModel mobileResourceTypeDataModel = inputModel
										.getMobileResourceTypeDataModel(mobileResourceTypeReference);
					...
					for (int k = 0; k < currentLoadNodeList.getLength(); k++) {
						...
						if (!currentLoadDetailNode.getNodeName().equals("#text")) {
							if (currentLoadDetailNode.getNodeName().equals("MOBILE_RESOURCE_REFERENCE")) {
								...
								MobileResourceDataModel mobileResource = inputModel
										.getMobileResourceDataModel(attributes.getNamedItem("refid").getNodeValue());
	...
	private InventoryDataModel getInventoryDataModel(Node inventory) {
		...
		for (int i = 0; i < inventoryDetails.getLength(); i++) {
			...
			// NamedNodeMap inventoryDetailNodeAttributes =
			// inventoryDetailNodeName.getAttributes();
								...
								for (int k = 0; k < supportedToolTypeDetails.getLength(); k++) {
									...
									if (!supportedToolTypeDetailNode.getNodeName().equals("#text")) {
										if (supportedToolTypeDetailNode.getNodeName().equals("TOOL_TYPE_REFERENCE")) {
											String toolTypeReference = null;
											NamedNodeMap referenceAttributes = supportedToolTypeDetailNode
													.getAttributes();
											if (referenceAttributes.getNamedItem("refid") != null) {
												toolTypeReference = referenceAttributes.getNamedItem("refid")
														.getNodeValue();
											}
											toolTypeDataModel = inputModel.getToolTypeDataModel(toolTypeReference);
										} else if (supportedToolTypeDetailNode.getNodeName()
												.equals("MAXIMUM_CAPACITY")) {
											if (supportedToolTypeDetailNode.getLastChild() != null) {
												String capacity = supportedToolTypeDetailNode.getLastChild()
														.getNodeValue();
					...
					for (int k = 0; k < currentLoadNodeList.getLength(); k++) {
						...
						if (!currentLoadDetailNode.getNodeName().equals("#text")) {
							if (currentLoadDetailNode.getNodeName().equals("TOOL_REFERENCE")) {
								NamedNodeMap attributes = currentLoadDetailNode.getAttributes();
								ToolDataModel tool = inputModel
										.getToolDataModel(attributes.getNamedItem("refid").getNodeValue());
								if (tool != null) {
			...
			for (int j = 0; j < jobDetailsNodeList.getLength(); j++) {
				if (!jobDetailsNodeList.item(j).getNodeName().equals("#text")) {
					if (jobDetailsNodeList.item(j).getNodeName().equals("ARRIVAL_DATE")) {
						arrivalDate = XMLUtil
								.getCalendarDateFromXMLNodeList(jobDetailsNodeList.item(j).getChildNodes());
						...
						TaskDataModel task = inputModel
								.getTaskDataModel(taskAttributes.getNamedItem("refid").getNodeValue());
						tasks.add(task);
						...
						workcenter = inputModel
								.getWorkcenterDataModel(workcenterAttributes.getNamedItem("refid").getNodeValue());
	...
	private Vector<ResourceDataModel> getResources() {
		...
		for (int i = 0; i < resourcesNodes.getLength(); i++) {
			...
			ResourceDataModel resource = new ResourceDataModel(resourceId, resourceName, description,
					resourceAvailability, setUpMatrixDataModel, endEffectorDataModel, properties);
			resources.add(resource);
		}
		return resources;
	}
	...
	private Calendar getPlanningInputDataModelStartDate() {
		...
		planStartCalendarDate.set(Integer.parseInt(planStartYear), Integer.parseInt(planStartMonth) - 1,
				Integer.parseInt(planStartDay), 0, 0, 0);
	...
	private Calendar getPlanningInputDataModelEndDate() {
		...
		planEndCalendarDate.set(Integer.parseInt(planEndYear), Integer.parseInt(planEndMonth) - 1,
				Integer.parseInt(planEndDay), 0, 0, 0);
		return planEndCalendarDate;
	}

	private boolean getContinueAssignmentsAfterPlanEndDate() {
		Node planNode = xmlPlanningInputDocument.getDocumentElement();
		Node continueAssignmentsAfterPlanEndDateAttribute = planNode.getAttributes()
				.getNamedItem("continueAssignmentsAfterPlanEndDate");
		...
		try {
			continueAssignmentsAfterPlanEndDate = Boolean.valueOf(continueAssignmentsAfterPlanEndDateString)
					.booleanValue();
		} catch (Exception ex) {
							...
							if (detailNode.getNodeName().equals("FROM_CODE")) {
								...
								if (detailNode.getLastChild() != null) {
									...
									try {
										...
										// Do not print anything this exception occured to wrong xml values. Continue
										// with defaults.
									}
