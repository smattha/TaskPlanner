...
public class FindLayout extends JFrame {
	 ...
	 *         is ready to run, running or finished running.
	 */
	...
	public static void main(String[] args) {
		...
		LOGGER.debug("flag: {} and assignments: {} and workcenters: {} and count: {} and total count: {}", FindLayout.flagForForLoop, FindLayout.sizeOfExistingAss, FindLayout.numOfWorkcenters, FindLayout.count, FindLayout.countTotal);

		ActivePassiveResourceScheduler activePassiveResourcesScheduler = new ActivePassiveResourceScheduler();
		...
		Vector<String> scheduledTask = activePassiveResourcesScheduler.getTaskSchedulerResults();
	...
	public FindLayout() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(FindLayout.class.getResource("/Images/ROBO-PARTNER_Logo.ICO")));
	...
	/**
	 * from this function is initialized the tcp/ip connection with the PS tool.
	 * It calls IMPACT, generates a layout and sends it back to PS. when the
	 * generation is finished the connection does not close and awaits a new
	 * call in order to generate another alternative.
	 * 
	 * @param args
	 */
	public void findLayoutClient() {
		...
		try {
			...
			//LOGGER.error("IO exception for input stream: {}", ExceptionUtils.getStackTrace(e));
		}
		...
		while ( waitForResponce ) {
			try {
				...
				if ( msgReceivedString.startsWith("{") ) {
					...
					//if ( jsonObject.getString(JsonPropertiesHelper.TYPE_PROPERTY).startsWith(JsonPropertiesHelper.TYPE_PROPERTY_VALUE_PROPERTIES) ) tasksWithProperties.add(jsonObject);
					//else if ( jsonObject.getString(JsonPropertiesHelper.TYPE_PROPERTY).startsWith(JsonPropertiesHelper.TYPE_PROPERTY_VALUE_TABLES) ) tasksWithTables.add(jsonObject);
					//else if ( jsonObject.getString(JsonPropertiesHelper.TYPE_PROPERTY).startsWith(JsonPropertiesHelper.TYPE_PROPERTY_VALUE_CELL) )
						//cellDimensions.add(jsonObject);
					outputStream.write("ok\n".getBytes());
				}
				else if ( msgReceivedString.startsWith("dataEnd") ) outputStream.write("start the procedure\n".getBytes());
				else if ( !msgReceivedString.startsWith("wait") ) {
					if ( msgReceivedString.startsWith("again") ) outputStream.write("start the procedure\n".getBytes());
						...
						if ( resultVector.size() > 0 ) {
							...
							msgReceivedString = new String(msgReceived, StandardCharsets.UTF_8);
							isFinished = true;
							if ( msgReceivedString.startsWith("ok") ) outputStream.write("end\n".getBytes());
						}
					}
				}
			}
				...
				//LOGGER.error("Excpetion while retrieving vector: {}", ExceptionUtils.getStackTrace(e));
			}
