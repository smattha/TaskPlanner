		...
		for ( int i = 0; i < sr; i++ ) {
			...
			for ( int j = 0; j < path.length; j++ ) {
				...
				for ( Assignment assignment : node.getNodeAssignments() ) {
					...
					double payloadLimit = Double.parseDouble(assignment.getResource().getResourceDataModel().getProperty("Payload (kg)"));
					double partWeight = Double.parseDouble(assignment.getTask().getTaskDataModel().getProperty("Weight (Kg)"));
					if ( assignment.getResource().getResourceDataModel().getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME).equals(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_HUMAN) ) {
						if ( partWeight > payloadLimit ) return 0;
						else partialPayloadTemp = partWeight;
					}
					...
					logger.trace(msg + " taskId: {} resourceId: {} {}: {}", taskId, resourceId, Payload.CRITERION_NAME, partialPayload);
				}
