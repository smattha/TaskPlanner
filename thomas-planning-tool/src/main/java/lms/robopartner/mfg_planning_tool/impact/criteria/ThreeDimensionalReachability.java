...
/**
 * This is mainly used to evaluate the plan’s calculated z-axis positioning. 
 * The reachability is used in order to provide a second way of initial evaluation of the z axis positioning of the components. 
 * The reachability of the robot is considered as being a circle around the robot 
 * having its centre placed at the 35% of the robot’s overall height from its bottom 
 * (that value has just been noticed as being close to true for the COMAU robots. 
 * Further investigation of this value should be done in order to be accepted as the correct one)
 * 
 * @author Jason
 *
 */
public class ThreeDimensionalReachability extends AbstractCriterion {
	...
	public ThreeDimensionalReachability(int internalDH, int internalMNA, int internalSR, PLANNINGINPUT aPlanninginput, Document planningInputDocument) {
        super();
...
//					        	if the task is a robot create a robotpositions rectangle
								if(taskDataModel.getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME).equals(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_ROBOT)){
					            	...
					            	RobotReachability.add(Integer.parseInt(taskDataModel.getProperty(MapToResourcesAndTasks.REACHABILTY_PROPERTY_NAME)));
					            	// TODO A Z-Height Property has to be added to all robots (or all components for that matter)
					            	RobotZDimension.add(Integer.parseInt(taskDataModel.getProperty(MapToResourcesAndTasks.HEIGHT_PROPERTY_NAME)));
					            }
...
//			            			Most of the robots have maximum reachability at an approximate height of 35% of its overall height
			            			...
			            			double distance = Math.sqrt(Math.pow((RobotPosition.x-PartsPosition.x), 2)+Math.pow((RobotPosition.y-PartsPosition.y), 2)+Math.pow((RobotZ-PartZ), 2));
		...
		logger.trace(msg + ThreeDimensionalReachability.CRITERION_NAME + "=" + resultCost	+ ". Solution found="+totalFoundPlacement);
		return resultCost;
