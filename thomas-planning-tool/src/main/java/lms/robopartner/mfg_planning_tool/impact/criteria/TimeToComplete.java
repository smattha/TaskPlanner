		...
		for (int i = 0; i < sr; i++) {
			...
			for (int j = 0; j < path.length; j++) {
				...
				for (Assignment assignment : node.getNodeAssignments()) {
					...
					double maxTimeToComplete = Math.sqrt(
							(Math.pow(MapParameters.MAP_HEIGHT, 2) + Math.pow(MapParameters.MAP_WIDTH, 2))) / 0.25;
					partialTimeToCompleteTemp = ((maxTimeToComplete - timeToCompleteMovement)
							/ (maxTimeToComplete - 0));
