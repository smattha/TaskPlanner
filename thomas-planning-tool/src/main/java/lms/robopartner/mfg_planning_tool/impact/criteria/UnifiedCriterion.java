...
/**
 * This criterion is used in order to call all others and calculate a final
 * weighted factor for the plan. This calls the below criteria: Ergonomy,
 * FloorSpace, InvestmentCost, Payload, SaturationLevel, TimeToComplete, Fatigue
 * 
 * @author Jason
 *
 */
		...
		for (int i = 0; i < sr; i++) {
			...
			for (int j = 0; j < path.length; j++) {
				...
				for (Assignment assignment : node.getNodeAssignments()) {
					...
					double[] Abstract = {
							new InvestmentCost(resourcesAndPartsMapingForSR).getValue(paths, helper, timeNow),
					...
					logger.trace(msg + " taskId=" + taskId + " resourceId=" + resourceId + "- partial"
							+ UnifiedCriterion.CRITERION_NAME + "=" + partialAreaCost);
				}
