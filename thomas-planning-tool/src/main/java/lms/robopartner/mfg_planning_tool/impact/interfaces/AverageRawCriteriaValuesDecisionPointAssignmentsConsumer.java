 ...
 *
 */
public class AverageRawCriteriaValuesDecisionPointAssignmentsConsumer
		implements DecisionPointAssignmentsConsumerInterface {
	private static org.slf4j.Logger logger = LoggerFactory
			.getLogger(AverageRawCriteriaValuesDecisionPointAssignmentsConsumer.class);
	...
	private void addCriteria(double[] criteriaValues) {
		...
		if (criteriaValues != null && criteriaValues.length > 0) {
			this.cumulativeCriteriaValues = AverageRawCriteriaValuesDecisionPointAssignmentsConsumer
					.initiliazeCriteriaVector(this.cumulativeCriteriaValues, criteriaValues.length);

			for (int i = 0; i < this.cumulativeCriteriaValues.length; i++) {
				logger.trace(msg + " Previous Value:" + this.cumulativeCriteriaValues[i] + " Next Value:"
	...
	/**
	 * @return null in the following cases: 1. The
	 *         {@link AverageRawCriteriaValuesDecisionPointAssignmentsConsumer#cumulativeCriteriaValues}
	 *         has not be initialized - no values have been accumulated yet. 2. The
	 *         criteriaValues provided consumed are of zero length.
	 * 
	 */
	public double[] getAverageCriteriaValues() {
		...
		if (this.cumulativeCriteriaValues != null && this.cumulativeCriteriaValues.length > 0) {
			averageValues = AverageRawCriteriaValuesDecisionPointAssignmentsConsumer
					.initiliazeCriteriaVector(averageValues, this.cumulativeCriteriaValues.length);
	 ...
	 * {@link AverageRawCriteriaValuesDecisionPointAssignmentsConsumer#getAverageCriteriaValues()}
	 * or null if the
	 * {@link AverageRawCriteriaValuesDecisionPointAssignmentsConsumer#getAverageCriteriaValues()}
	 * is null
	 * 
