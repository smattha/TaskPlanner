...
public class AreaCost extends AbstractCriterion {
	...
	public double getValue(Vector<TreeNode[]> paths, PlanHelperInterface helper, Calendar timeNow) {
		...
		for ( TreeNode[] path : paths ) {
			for ( TreeNode treeNode : path ) {
				...
				for ( Assignment assignment : node.getNodeAssignments() ) {
					...
					double widthTask = Convert.getDouble(taskDataModel.getProperty(MapToResourcesAndTasks.WIDTH_PROPERTY_NAME));
					double heightTask = Convert.getDouble(taskDataModel.getProperty(MapToResourcesAndTasks.LENGTH_PROPERTY_NAME));
					ResourceDataModel resourceDataModel = assignment.getResource().getResourceDataModel();
					alternative += taskId + " X: " + resourceDataModel.getProperty(MapToResourcesAndTasks.LOCATION_X_PROPERTY_NAME) + " Y: "
							+ resourceDataModel.getProperty(MapToResourcesAndTasks.LOCATION_Y_PROPERTY_NAME);
					double resourceX = Convert.getDouble(resourceDataModel.getProperty(MapToResourcesAndTasks.LOCATION_X_PROPERTY_NAME));
					double resourceY = Convert.getDouble(resourceDataModel.getProperty(MapToResourcesAndTasks.LOCATION_Y_PROPERTY_NAME));

					partialAreaCost = widthTask * heightTask + resourceX * resourceY;
					areaCostSum += partialAreaCost;

					LOGGER.trace(msg + " taskId=" + taskId + " resourceId=" + resourceId + "- partial" + AreaCost.CRITERION_NAME + "="
							+ partialAreaCost);
				}
			}
		...
		if ( areaCostSum < 0 ) areaCostSum = -areaCostSum;
