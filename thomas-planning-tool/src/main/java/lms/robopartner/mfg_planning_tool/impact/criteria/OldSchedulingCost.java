...
/**
 * @author Spyros
 * Do not use, anywhere. Kept for reference.
 */
@Deprecated
public class OldSchedulingCost extends AbstractCriterion {
	private static org.slf4j.Logger logger = LoggerFactory
		.getLogger(OldSchedulingCost.class);
	...
	public double getValue(Vector<TreeNode[]> paths,
		...
		for (TreeNode[] path : paths) {
			...
			LayerNode aTempLayerNodeToAccessEnclosingType=new LayerNode(new HashMap<ResourceSimulator, Vector<TaskSimulator>>(), new Vector<LayerNode.Assignment>(), new Vector<ResourceSimulator>());
			
			//Now tempTaskIdsList contains the ids of all the tasks that have not been assigned.
			for(String aTaskId:tempTaskIdsList){
				logger.trace(msg + "Sample " + srIndex + " of " + sr + ". task not assigned:"+aTaskId);
				Point aPoint=LayoutEvaluator.getAValidPointForResource(allTaskIdsReference.get(aTaskId), allAssignments,this.maxTries);
				if(aPoint!=null){
				//create a fake assignment
					
					Assignment anAssignment= aTempLayerNodeToAccessEnclosingType.new Assignment(new ResourceSimulator(allResourcesIdsReference.get(MapParameters.getIDfromLocation(aPoint))), new TaskSimulator(allTaskIdsReference.get(aTaskId)));
					allAssignments.add(anAssignment);
				}
				else{
//					issue a warning
					logger.debug(msg+"Sample " + srIndex + " of " + sr + "."+"Unable to find a placement for task:"+aTaskId);
			...
			for (TreeNode treeNode : path) {
				...
				for (Assignment assignment : layerNode.getNodeAssignments()) {
                    ...
                    TaskDataModel taskDataModel = assignment.getTask()
						.getTaskDataModel();
		...
		logger.trace(msg + OldSchedulingCost.CRITERION_NAME + "=" + resultCost
			+ ". Solution found="+totalFoundPlacement);
		return resultCost;
