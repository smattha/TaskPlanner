...
/**
 * this criterion calculates the operational cost for the assignment.
 * This is calculated differently if the resource executing the task is a human or a robot.
 * In the case of the robot, there is a cost per part that has been calculated and is used
 * (for the calculated of this cost/part for the robot please refer to the documentation.docx)
 * In the case of the human operator, the time that requires a task is calculated and is
 * cross-referenced against its annual or monthly payment.
 * 
			...
			for ( int j = 0; j < path.length; j++ ) {
				...
				for ( Assignment assignment : node.getNodeAssignments() ) {
					...
					if ( assignment.getResource().getResourceDataModel().getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME).equals(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_ROBOT) ) {
						...
						investmentCost = Double.parseDouble(assignment.getResource().getResourceDataModel().getProperty("Cost (euro)"));
					}
						...
						double costPerSecond = Double.parseDouble(assignment.getResource().getResourceDataModel().getProperty("Cost (euro)"));
						double centerXTask;
						double centerYTask;
						...
						// 2)+Math.pow((centerYResource-centerYTask), 2)+Math.pow((zResource-zTask), 2));
					...
					//partialInvestmentCostTemp = ((ReschedulingPlanningGenerator.MAX_INVESTMENT_COST - investmentCost) / (ReschedulingPlanningGenerator.MAX_INVESTMENT_COST - ReschedulingPlanningGenerator.MIN_INVESTMENT_COST));
