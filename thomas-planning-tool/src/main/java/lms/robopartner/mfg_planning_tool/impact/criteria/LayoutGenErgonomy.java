...
/**
 * An initial ergonomy evaluation based on only to parameters of the NIOSH
 * equation, the Vertical Multiplier and the Distance Multiplier. It is used in
 * order to have a somewhat good layout concerning the z-axis.
 * 
...
public class LayoutGenErgonomy extends AbstractCriterion {
	...
	public LayoutGenErgonomy(int internalDH, int internalMNA, int internalSR, PLANNINGINPUT aPlanninginput,
	...
	public double getValue(Vector<TreeNode[]> paths, PlanHelperInterface helper, Calendar timeNow) {
			...
			for (TreeNode treeNode : path) {
				...
				for (Assignment assignment : layerNode.getNodeAssignments()) {
					...
					// if the task is a task create a task position's rectangle
						...
						// TODO A property for the base part should be entered in the planning
						// generator. That can also
						// be done by
						// testing whether this is the first task, as the base part will always be
						// manipulate as the
						// first task.
						...
						if (taskDataModel.getProperty(MapToResourcesAndTasks.BASE_PART_PROPERTY_NAME) != null
								&& taskDataModel.getProperty(MapToResourcesAndTasks.BASE_PART_PROPERTY_NAME)
										.equals(true)) {
							BasePartPositionZ = Integer.parseInt(
									taskDataModel.getProperty(MapToResourcesAndTasks.LOCATION_Z_PROPERTY_NAME));
						}
					}
				}
				...
				if (!PartsPositions.isEmpty()) {
					// All distances below have been expressed in mm this should possibly be changed
					// depending on the
		...
		logger.trace(msg + LayoutGenErgonomy.CRITERION_NAME + "=" + resultErgonomy + ". Solution found="
				+ totalFoundPlacement);
