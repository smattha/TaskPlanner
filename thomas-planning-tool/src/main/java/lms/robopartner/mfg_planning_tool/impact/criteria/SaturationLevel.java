...
/**
 * The saturation is the value of the time that the robot is occupied by a task
 * compared to the overall time of the assignment.
 * 
...
//                      double distance = Math.sqrt(Math.pow((centerXResource-centerXTask), 2)+Math.pow((centerYResource-centerYTask), 2)+Math.pow((zResource-zTask), 2));
						...
						if (assignment.getResource().getResourceDataModel()
							...
							/*
							 * This has to be specific to the resource. Saturation levels should be created
							 * for all resources and the minimum will be examined as a criterion
							 */
					...
					logger.trace(msg + " taskId=" + taskId + " resourceId=" + resourceId
							+ SaturationLevel.CRITERION_NAME + "=" + partialSaturationLevel);
				}
