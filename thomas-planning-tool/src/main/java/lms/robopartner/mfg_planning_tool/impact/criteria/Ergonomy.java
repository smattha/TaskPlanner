...
/**
 * Used for the evaluation of the ergonomy of the tasks executed by the human
 * operator. If a task is executed by the robot, its value is maximized. The
 * evaluation is based on NIOSH lifting equation
...
//                        partialAreaCost = Double.MAX_VALUE;
						...
						/*
						 * vertical multiplier is dependent on the height of the part to be picked up.
						 * for 30inch=1
						 */
						double VM = 1;
						/*
						...
						double DM = 1;
						/*
						 * Frequency multiplier is dependent on the hours of work/day and the lifts/min
						 * (production rate). possibly consider using user input for 8hours and 1
						 * lift/min=0.75
						 */
						double FM = 0.75;
						/*
						 * Coupling multiplier used to identify the robustness of the grip that will be
						 * achieved
						 */
						double CM = 0.9;
						double centerXTask;
						...
						/*
						 * Horizontal multiplier is dependent on the distance of the part to be picked
						 * up and the operator's ankles
						 */
						double HM = 0;
						if (distance <= 0.70) {
							if (distance <= 0.25) {
						...
						if (assignment.getResource().getResourceDataModel()
							...
							if (firstTask) {
								...
								/*
								 * Assymetry multiplier is dependent on the rtation the body of the operator is
								 * asked to perform. if this was the first task, the operator is considered
								 * looking at the part and no rotation of the body will occur. in any other case
								 * the rotation is calculated as below.
								 */
								double PartVectorX = centerXTask - basePartX;
								double PartVectorY = centerYTask - basePartY;
								double angle = (vectorX * PartVectorX + vectorY * PartVectorY)
										/ (Math.sqrt(Math.pow(vectorX, 2) + Math.pow(vectorY, 2))
												* (Math.sqrt(Math.pow(PartVectorX, 2) + Math.pow(PartVectorY, 2))));
								if ((angle >= 0) && (angle < 15)) {
									AM = 1;
								}
								...
								/*
								 * vertical multiplier is dependent on the height of the part to be picked up
								 * relative to the floor. in the best case scenario the part is placed in a
								 * height that corresponds to 30 inches = 889 mm
								 */
								// TODO the below commented line(s) are the 3D criteria changes
//                    				if (zTask<=0){
//			            				VM = 0;
...
//									}
								 ...
								 * it takes greater value the closer it is to 0. at a distanc greater than 70
								 * inch = 1778 mm the value distance multiplier takes the value of 0
								 */
...
//									}
					...
					logger.trace(msg + " taskId=" + taskId + " resourceId=" + resourceId + Ergonomy.CRITERION_NAME + "="
							+ partialErgonomy);
				}
