...
public class DemoPlanningGenerator {
	...
	public static PROPERTIES getPROPERTIES(String resourceType, double payloadKg, double costEuro, double floorSpaceM2,
			double speedMMS, int width, int height, double reachability, int zheight) {
	...
	public static TASKS getTasks(int maxTasks) {
		...
		for (int i = 0; i < maxTasks; i++) {
			...
			TASK aTask = MapToResourcesAndTasks.getTask("Task Axle Load" + id, "Axle " + id, id + "",
					(int) Math.ceil(500 / 500.0), (int) Math.ceil(600 / 500.0), (int) Math.ceil(600 / 500.0), 25.0,
					true, 1);

			id = IDGenerator.getNewID();
			TASK bTask = MapToResourcesAndTasks.getTask("Task Drum" + id, "Drum " + id, id + "",
					(int) Math.ceil(1400 / 500.0), (int) Math.ceil(700 / 500.0), (int) Math.ceil(1400 / 500.0), 11.0,
					false, 1);

			id = IDGenerator.getNewID();
			TASK cTask = MapToResourcesAndTasks.getTask("Task Cable" + id, "Cable " + id, id + "",
					(int) Math.ceil(100 / 500.0), (int) Math.ceil(100 / 500.0), (int) Math.ceil(100 / 500.0), 1.0,
					false, 1);

			id = IDGenerator.getNewID();
			TASK dTask = MapToResourcesAndTasks.getTask("Task Screws" + id, "Screws " + id, id + "",
					(int) Math.ceil(100 / 500.0), (int) Math.ceil(100 / 500.0), (int) Math.ceil(100 / 500.0), 1.0,
					false, 1);
	...
	public static RESOURCES getResources(int maxResources) {
		...
		for (int i = 0; i < maxResources; i++) {
			...
			PROPERTIES props1 = DemoPlanningGenerator.getPROPERTIES(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_ROBOT,
					370.0, 20000.0, (int) Math.ceil((1400 * 2500) / (500.0 * 500.0)), (1000.0 / 500.0),
					(int) Math.ceil(1400 / 500.0), (int) Math.ceil(2500 / 500.0), (3000 / 500.0), 1);
			RESOURCE aResource1 = MapToResourcesAndTasks.getResource("Resource" + id, "The Resource " + id, id + "",
					props1);
...
//            RESOURCE aResource3 = MapToResourcesAndTasks.getResource("Resource" + id, "The Resource " + id, id + "", props3);
			...
			id = IDGenerator.getNewID();
			// TODO set suitable height
			PROPERTIES props2 = DemoPlanningGenerator.getPROPERTIES(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_HUMAN,
					11.0, 200.0, (int) Math.ceil((500 * 600) / (5000 * 500.0)), (500.0 / 500.0),
					(int) Math.ceil(500 / 500.0), (int) Math.ceil(600 / 500.0), MapParameters.MAP_WIDTH, 1);
			RESOURCE aResource2 = MapToResourcesAndTasks.getResource("Resource" + id, "The Resource " + id, id + "",
					props2);
...
//            resources.getRESOURCE().add(aResource3);
			...
			for (TASK aTask : theTasks.getTASK()) {
				if (!(aTask.getNAME().equals("Axle 2"))) {
					if (aResource.getNAME().equals("The Resource 7")) {
						TASKSUITABLERESOURCE tasksuitableresource = MapToResourcesAndTasks
								.getTaskSuitableResource(aResource, aTask, setupcode, operationTimePerBatchSeconds);
	...
	public static PLANNINGINPUT getResourcesAndTasksOriginalDemoPlanninginput() {
		...
		aPlanninginput.setTASKSUITABLERESOURCES(DemoPlanningGenerator.getTaskSuitableResources(theResources, theTasks,
