...
public class ResultScheduler3D {
	...
	public Vector<String> resultScheduler(Vector<AssignmentDataModel> assignments, Map<String, Point> resourcesAndPartsMapingForSR) {
		...
		for ( Date date : allDispathesDates ) {
			...
			for ( AssignmentDataModel assignment : dateAssignments ) {
				output = "result@" + assignment.getTaskDataModel().getTaskName() + "@" + assignment.getResourceDataModel().getResourceName() + "@"
