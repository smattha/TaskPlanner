...
public class LayoutEvaluator {
	private static org.slf4j.Logger logger = LoggerFactory.getLogger(LayoutEvaluator.class);

	public static boolean isSuitabilityValid(Point resourcePoint, Dimension taskDimension, int taskReachability,
			Vector<AssignmentDataModel> existingAssignements) {
		...
		if (mapRectangle.contains(assignementRectangle)) {
			...
			Rectangle currentTaskReachabilityRectangle = LayoutEvaluator.getReachabilityRectangle(assignementRectangle,
					taskReachability);

			for (AssignmentDataModel anAssignement : existingAssignements) {
				Rectangle theOtherRectangle = DataModelToAWTHelper.createRectangle(anAssignement.getResourceDataModel(),
						anAssignement.getTaskDataModel());
				...
				if (!isSuitable) {
					logger.trace(msg + "Rectangle=" + assignementRectangle + " isSuitable=" + isSuitable
							+ " [intersects Renctangle:" + theOtherRectangle + "]");
					break;
				} else {
					// else check reachability

					Rectangle anotherReachabilityRectangle = LayoutEvaluator.getReachabilityRectangle(theOtherRectangle,
							Integer.parseInt(anAssignement.getTaskDataModel()
									.getProperty(MapToResourcesAndTasks.REACHABILTY_PROPERTY_NAME)));
					if (LayoutEvaluator.areWithinReachability(currentTaskReachabilityRectangle,
							anotherReachabilityRectangle)) {
			...
			isSuitable = isSuitable && (existingAssignements.size() == 0
					|| (assignmentsWithinReachability > 0 && assignmentshumanPositionNotBetweenRobotAndParts > 0));

		} else {
			// the boundaries of the tasks-object fall outside the maps area.
			isSuitable = false;
			logger.trace(msg + "Rectangle=" + assignementRectangle + " isSuitable=" + isSuitable + " [out of bounds]");
		}
		if (isSuitable) {
			logger.trace(msg + "Rectangle=" + assignementRectangle + " isSuitable=" + isSuitable);
		}
	 ...
	 * @return
	 */
	public static boolean isSuitabilityValid(Point resourcePoint, Dimension taskDimension,
			Vector<Assignment> existingAssignements, int taskReachability) {
		...
		if (mapRectangle.contains(assignementRectangle)) {
			...
			for (Assignment anAssignement : existingAssignements) {
				Rectangle theOtherRectangle = DataModelToAWTHelper.createRectangle(
						anAssignement.getResource().getResourceDataModel(), anAssignement.getTask().getTaskDataModel());
				...
				if (!isSuitable) {
					...
					Rectangle anotherReachabilityRectangle = LayoutEvaluator.getReachabilityRectangle(theOtherRectangle,
							Integer.parseInt(anAssignement.getTask().getTaskDataModel()
									.getProperty(MapToResourcesAndTasks.REACHABILTY_PROPERTY_NAME)));
					if (LayoutEvaluator.areWithinReachability(currentTaskReachabilityRectangle,
							anotherReachabilityRectangle)) {
						assignmentsWithinReachability++;
	 ...
	 * @return
	 */
	public static boolean isSuitabilityValid(ResourceSimulator resource, TaskSimulator task,
			Vector<AssignmentDataModel> existingAssignements) {

		Point resourcePoint = DataModelToAWTHelper.getPointFromResource(resource);
		boolean isXYIndependentAndReachabilityMatching = LayoutEvaluator.isSuitabilityValid(resourcePoint,
				DataModelToAWTHelper.getDimensionFromTask(task),
				Integer.parseInt(task.getTaskDataModel().getProperty(MapToResourcesAndTasks.REACHABILTY_PROPERTY_NAME)),
	...
	public static Rectangle getReachabilityRectangle(Rectangle aTaskRectangle, int reachability) {
		...
		Rectangle aReachabilityRectanle = new Rectangle(aTaskRectangle.getLocation(),
				new Dimension(reachabilityWidth, reachabilityHeight));

		aReachabilityRectanle.setLocation(
				aTaskRectangle.x + (int) ((aTaskRectangle.getCenterX() - aReachabilityRectanle.getCenterX())),
				aTaskRectangle.y + (int) ((aTaskRectangle.getCenterY() - aReachabilityRectanle.getCenterY())));
	 ...
	 * @return
	 */
	public static boolean areWithinReachability(Rectangle aReachabilityRectangle,
			Rectangle anotherReachabilityRectangle) {
		String msg = ".areWithinReach() ";
		if (aReachabilityRectangle == null || anotherReachabilityRectangle == null) {
			throw new RuntimeException(
					msg + "aReachabilityRectangle or  anotherReachabilityRectangle should never be null.");
		}

		boolean areWithinReach = aReachabilityRectangle.intersects(anotherReachabilityRectangle)
				|| aReachabilityRectangle.contains(anotherReachabilityRectangle)
				|| anotherReachabilityRectangle.contains(aReachabilityRectangle);
		logger.trace(msg + " rectangle=" + aReachabilityRectangle + " and rectangle=" + anotherReachabilityRectangle
				+ " areWithinReach=" + areWithinReach);
	...
	public static boolean humanPositionNotBetweenRobotAndParts(Vector<AssignmentDataModel> existingAssignements) {
		...
		for (AssignmentDataModel anAssignement : existingAssignements) {
			...
			if (anAssignement.getTaskDataModel().getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME)
					.equals(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_ROBOT)) {
				Rectangle RobotRectangle = DataModelToAWTHelper.createRectangle(anAssignement.getResourceDataModel(),
						anAssignement.getTaskDataModel());
				RobotPositions.add(RobotRectangle);
			}
			// if the task is a human create a humanpositions rectangle
			else if (anAssignement.getTaskDataModel().getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME)
					.equals(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_HUMAN)) {
				Rectangle HumanRectangle = DataModelToAWTHelper.createRectangle(anAssignement.getResourceDataModel(),
						anAssignement.getTaskDataModel());
			...
			// if the task is a task create a taskpositions rectangle
				...
				Rectangle PartRectangle = DataModelToAWTHelper.createRectangle(anAssignement.getResourceDataModel(),
						anAssignement.getTaskDataModel());
			...
			for (Rectangle HumanPosition : HumanPositions) {
				for (Rectangle RobotPosition : RobotPositions) {
					for (Rectangle PartsPosition : PartsPositions) {
						// check whether the part is positioned right from the robot in a height same as
						// the robot's
						if ((PartsPosition.x >= RobotPosition.x + RobotPosition.width)
								&& ((PartsPosition.y >= RobotPosition.y) || (PartsPosition.y
										+ PartsPosition.height <= RobotPosition.y + RobotPosition.height))) {
							// check whether the human is positioned right from the robot in a height same
							// as the robot's
							if ((HumanPosition.x >= RobotPosition.x + RobotPosition.width)
									&& ((HumanPosition.y >= RobotPosition.y) || (HumanPosition.y
											+ HumanPosition.height <= RobotPosition.y + RobotPosition.height))) {
								...
								if (HumanPosition.x + HumanPosition.width <= PartsPosition.x) {
									...
									logger.trace(msg + " humanPositionNotBetweenRobotAndParts="
											+ humanPositionNotBetweenRobotAndParts);
									return humanPositionNotBetweenRobotAndParts;
								}
							}
							// check whether the part is positioned right from the robot in a height same as
							// the robot's
						} else if ((PartsPosition.x + PartsPosition.width <= RobotPosition.x)
								&& ((PartsPosition.y >= RobotPosition.y) || (PartsPosition.y
										+ PartsPosition.height <= RobotPosition.y + RobotPosition.height))) {
							// check whether the human is positioned right from the robot in a height same
							// as the robot's
							if ((HumanPosition.x + HumanPosition.width <= RobotPosition.x)
									&& ((HumanPosition.y >= RobotPosition.y) || (HumanPosition.y
											+ HumanPosition.height <= RobotPosition.y + RobotPosition.height))) {
								// whether the human is closer to the robot than the part
							...
							// check whether the part is positioned downwards from the robot in a horizontal
							// position same as the robot's
						} else if ((PartsPosition.y >= RobotPosition.y + RobotPosition.height)
								&& ((PartsPosition.x >= RobotPosition.x) || (PartsPosition.x
										+ PartsPosition.width <= RobotPosition.x + RobotPosition.width))) {
							// check whether the human is positioned downwards from the robot in a
							// horizontal position same as the robot's
							if ((HumanPosition.y >= RobotPosition.y + RobotPosition.height)
									&& ((HumanPosition.x >= RobotPosition.x) || (HumanPosition.x
											+ HumanPosition.width <= RobotPosition.x + RobotPosition.width))) {
								// whether the human is closer to the robot than the part
							...
							// check whether the part is positioned upwards from the robot in a horizontal
							// position same as the robot's
						} else if ((PartsPosition.y + PartsPosition.height <= RobotPosition.y)
								&& ((PartsPosition.x >= RobotPosition.x) || (PartsPosition.x
										+ PartsPosition.width <= RobotPosition.x + RobotPosition.width))) {
							// check whether the human is positioned upwards from the robot in a horizontal
							// position same as the robot's
							if ((HumanPosition.y + HumanPosition.height <= RobotPosition.y)
									&& ((HumanPosition.x >= RobotPosition.x) || (HumanPosition.x
											+ HumanPosition.width <= RobotPosition.x + RobotPosition.width))) {
								// whether the human is closer to the robot than the part
								if (HumanPosition.y >= PartsPosition.y) {
						...
						} else if ((PartsPosition.x >= RobotPosition.x + RobotPosition.width)
								&& (PartsPosition.y + PartsPosition.height <= RobotPosition.y)) {
							// check whether the human is positioned upwards and right from the robot
							if ((HumanPosition.x >= RobotPosition.x + RobotPosition.width)
									&& (HumanPosition.y + HumanPosition.height <= RobotPosition.y)) {
								double PartsDistanceFromRobot = Math
										.sqrt(Math.pow((PartsPosition.x - RobotPosition.x), 2)
												+ Math.pow((PartsPosition.y - RobotPosition.y), 2));
								double HumanDistanceFromRobot = Math
										.sqrt(Math.pow((HumanPosition.x - RobotPosition.x), 2)
												+ Math.pow((HumanPosition.y - RobotPosition.y), 2));
								// whether the human is closer to the robot than the part
							...
							// check whether the part is positioned downwards and right from the robot
								...
								&& (PartsPosition.y >= RobotPosition.y + RobotPosition.height)) {
							// check whether the human is positioned downwards and right from the robot
							if ((HumanPosition.x >= RobotPosition.x + RobotPosition.width)
									&& (HumanPosition.y >= RobotPosition.y + RobotPosition.height)) {
								double PartsDistanceFromRobot = Math
										.sqrt(Math.pow((PartsPosition.x - RobotPosition.x), 2)
												+ Math.pow((PartsPosition.y - RobotPosition.y), 2));
								double HumanDistanceFromRobot = Math
		...
		// boolean humanPositionNotBetweenRobotAndParts =
		// aReachabilityRectangle.intersects(anotherReachabilityRectangle) ||
		// aReachabilityRectangle.contains(anotherReachabilityRectangle) ||
		// anotherReachabilityRectangle.contains(aReachabilityRectangle);
	 ...
	 * existingAssignements. Tries using three different algorithms. The first
	 ...
	 * @return
	 */
	public static Point getAValidPointForResource(TaskDataModel task, Vector<Assignment> existingAssignements,
			int maxTries) {
	...
	/**
	 * Randomly selects and checks maxTries squares in the space of the map. Returns
	 * a point if it is valid, null if no point is found.
	 * 
	 * @param task
	 * @param existingAssignements
	 * @param maxTries
	 * @return
	 */
	private static Point getAValidPointForResourceRandom(TaskDataModel task, Vector<Assignment> existingAssignements,
			int maxTries) {
		...
		for (int i = 0; i < maxTries; i++) {
			...
			returnPoint.setLocation(aRandom.nextInt(MapParameters.MAP_WIDTH + 1),
					aRandom.nextInt(MapParameters.MAP_HEIGHT + 1));

			if (LayoutEvaluator.isSuitabilityValid(returnPoint, taskDimensions, existingAssignements,
					taskReachability)) {
				return returnPoint;
			}
	...
	/**
	 * Equally distributes maxTries squares in the space of the map. Returns a point
	 * if it is valid, null if no point is found.
	 * 
	 * @param task
	 * @param existingAssignements
	 * @param maxTries
	 * @return
	 */
	private static Point getAValidPointForResourceExtensive(TaskDataModel task, Vector<Assignment> existingAssignements,
			int maxTries) {
		Point returnPoint = new Point(0, 0);
	...
	/**
	 * Uses a heyristic to check the available positions of a Task. If the heuristic
	 * succeeds it returns a Point. Will search 8 locations per existing assignment.
	 * 
	 * @param task
	 * @param existingAssignements
	 * @return
	 */
	private static Point getAValidPointForResourceHeuristic(TaskDataModel task,
			Vector<Assignment> existingAssignements) {
		...
		for (Assignment anAssignment : existingAssignements) {
			Rectangle assignementRectangle = DataModelToAWTHelper.createRectangle(anAssignment.getResource(),
					anAssignment.getTask());
			...
			reason = "down right";
			returnPoint = new Point(assignementRectangle.x + assignementRectangle.width - taskDimensions.width,
					assignementRectangle.y + assignementRectangle.height);
			if (LayoutEvaluator.isSuitabilityValid(returnPoint, taskDimensions, existingAssignements,
					taskReachability)) {
				break;
			}
			reason = "left down";
			returnPoint = new Point(assignementRectangle.x - taskDimensions.width,
					assignementRectangle.y + assignementRectangle.height - taskDimensions.height);
			if (LayoutEvaluator.isSuitabilityValid(returnPoint, taskDimensions, existingAssignements,
					taskReachability)) {
				break;
			}
			...
			reason = "up right";
			returnPoint = new Point(assignementRectangle.x + assignementRectangle.width - taskDimensions.width,
					assignementRectangle.y - taskDimensions.height);
			if (LayoutEvaluator.isSuitabilityValid(returnPoint, taskDimensions, existingAssignements,
					taskReachability)) {
				break;
			}
			reason = "right down";
			returnPoint = new Point(assignementRectangle.x + assignementRectangle.width,
					assignementRectangle.y + assignementRectangle.height - taskDimensions.height);
			if (LayoutEvaluator.isSuitabilityValid(returnPoint, taskDimensions, existingAssignements,
					taskReachability)) {
				break;
