...
/**
 * @author Spyros Helper class to provide conversion functionality for INTRALOGISTICS
 */
public abstract class IntralogisticsDataModelHelper {
    ...
    public final static boolean isResourceIMAU(ResourceDataModel resource) {
        boolean virtualIMAUId = resource.getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME).equalsIgnoreCase(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_IMAU);
    ...
    public final static Integer getVirtualIMAUAssignment(TaskDataModel task) {
        ...
        logger.trace(msg + "TaskId=" + task.getTaskId() + MapToResourcesAndTasks.VIRTUAL_IMAU_PROPERTY_NAME + "=" + virtualIMAUId);
        return virtualIMAUId;
    }

    /**
     * @param task
     * @return the difference in the current number of boxes (that is the boxes that the IMAU is carrying) after performing the task.
     * This is +1 if it is loading a box and -1 if it is unloading a box. 
     */
    public final static Integer getDifferenceInCurrentNumberOfBoxesForTask(TaskDataModel task) {
        ...
        if (task.getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME).equalsIgnoreCase(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_LOADING)) {
            differenceInCurrentNumberOfBoxesForTask = +1;
        }
        else
            if (task.getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME).equalsIgnoreCase(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_UNLOADING)) {
                differenceInCurrentNumberOfBoxesForTask = -1;
            }
    ...
    public final static Integer getIMAUMaxNumberOfBoxes(ResourceDataModel resource) {
        Integer imauMaxNumberOfBoxes = (int) Double.parseDouble(resource.getProperty(MapToResourcesAndTasks.IMAU_MAX_NUMBER_OF_BOXES));
    ...
    public final static Integer getIMAUInitialNumberOfBoxes(ResourceDataModel resource) {
        Integer imauInitialNumberOfBoxes = (int) Double.parseDouble(resource.getProperty(MapToResourcesAndTasks.IMAU_INITIAL_NUMBER_OF_BOXES));
