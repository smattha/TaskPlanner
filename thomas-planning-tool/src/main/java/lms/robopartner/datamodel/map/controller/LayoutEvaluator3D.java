...
public class LayoutEvaluator3D {
	 ...
	 * @return
	 */
	public boolean isSuitabilityValid3D(ResourceSimulator resource, TaskSimulator task,
			Vector<AssignmentDataModel> existingAssignments, PLANNINGINPUT startingPlanningInput) {
		// Get box from task
		...
		boolean isXYZIndependentAndReachabilityMatching = this.isSuitabilityValid3D(
				Integer.parseInt(task.getTaskDataModel().getProperty(MapToResourcesAndTasks.REACHABILTY_PROPERTY_NAME)),
				existingAssignments, startingPlanningInput, task.getTaskId(), task, resource);
	...
	public boolean isSuitabilityValid3D(int taskReachability, Vector<AssignmentDataModel> existingAssignments,
			PLANNINGINPUT startingPlanningInput, String taskId, TaskSimulator taskSimulator,
		...
		LOGGER.debug("{}\naRectangle: \n\t\tx: {} y: {} \n\t\tw: {} h: {}", msg, assignmentRectangle.getBounds().x,
				assignmentRectangle.getBounds().y, assignmentRectangle.getBounds().width,
				assignmentRectangle.getBounds().height);
		if (!angle.equals("180") && !angle.equals("360") && !angle.equals("0"))
			assignmentRectangle = DataModelToAWTHelper.rotateShape(Integer.parseInt(angle), assignmentRectangle,
					resourcePoint.x, resourcePoint.y,
					taskSimulator.getTaskDataModel().getProperty(MapToResourcesAndTasks.SHAPE_TYPE_NAME));
		...
		if (mapRectangle.contains(assignmentRectangle.getBounds())) {
			for (AssignmentDataModel anAssignment : existingAssignments) {
				angle = anAssignment.getResourceDataModel()
				...
				Shape assignedRectangle = DataModelToAWTHelper.createShape(
						anAssignment.getTaskDataModel().getProperty(MapToResourcesAndTasks.SHAPE_TYPE_NAME),
						anAssignment.getTaskDataModel(), anAssignment.getResourceDataModel());
				if (!angle.equals("180") && !angle.equals("360") && !angle.equals("0"))
					assignedRectangle = DataModelToAWTHelper.rotateShape(Integer.parseInt(angle), assignedRectangle,
							resourcePoint.x, resourcePoint.y,
							anAssignment.getTaskDataModel().getProperty(MapToResourcesAndTasks.SHAPE_TYPE_NAME));
				LOGGER.debug(msg + "\n\tresource: {} \n\ttask: {} \n\tassignmentResource: {} \n\tassignmentTask: {}",
						resource.getResourceDataModel().getResourceId(),
						taskSimulator.getTaskDataModel().getProperty(MapToResourcesAndTasks.WIDTH_PROPERTY_NAME) + " "
								+ taskSimulator.getTaskDataModel()
										.getProperty(MapToResourcesAndTasks.LENGTH_PROPERTY_NAME),
						anAssignment.getResourceDataModel().getResourceId(),
						anAssignment.getTaskDataModel().getProperty(MapToResourcesAndTasks.WIDTH_PROPERTY_NAME) + " "
								+ anAssignment.getTaskDataModel()
										.getProperty(MapToResourcesAndTasks.LENGTH_PROPERTY_NAME));
				LOGGER.debug("\naRectangle: \n\t\tx: {} \n\t\tw: {} \nbRectangle: \n\t\tx: {} \n\t\tw: {}",
						assignmentRectangle.getBounds().x + " y: " + assignmentRectangle.getBounds().y,
						assignmentRectangle.getBounds().width + " h: " + assignmentRectangle.getBounds().height,
						assignedRectangle.getBounds().x + " y: " + assignedRectangle.getBounds().y,
						assignedRectangle.getBounds().width + " h: " + assignedRectangle.getBounds().height);
		...
		if (isSuitable && !taskSimulator.getTaskDataModel().getProperty(MapToResourcesAndTasks.TYPE_PROPERTY_NAME)
				.equals(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_ROBOT))
			isSuitableForReachability = areWithinReachability(startingPlanningInput, existingAssignments,
					assignmentRectangle, taskId);

		// logger.trace(msg + "Rectangle = {}, isSuitable && isSuitableForReachability =
		// {}, ", assignmentRectangle,
	...
	public Rectangle getReachabilityRectangle(Rectangle aTaskRectangle, int reachability) {
		...
		Rectangle aReachabilityRectangle = new Rectangle(aTaskRectangle.getLocation(),
				new Dimension(reachabilityWidth, reachabilityHeight));

		aReachabilityRectangle.setLocation(
				aTaskRectangle.x + (int) ((aTaskRectangle.getCenterX() - aReachabilityRectangle.getCenterX())),
				aTaskRectangle.y + (int) ((aTaskRectangle.getCenterY() - aReachabilityRectangle.getCenterY())));
	...
	public boolean areWithinReachability(PLANNINGINPUT aPlanningInput, Vector<AssignmentDataModel> existingAssignments,
			Shape taskRectangle, String taskId) {
		if (prevId.equals(""))
			prevId = taskId;
			...
			if (aTask.getTASKREFERENCE().getRefid().equals(taskId)) {
				...
				for (RESOURCE aResource : aPlanningInput.getRESOURCES().getRESOURCE()) {
					if (resourceId.equals(aResource.getId())) {
						// logger.debug(msg + "resourceName: {} TypePropertValue: {}",
						// aResource.getNAME(),
						// MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_HUMAN);
						if (aResource.getNAME().startsWith(MapToResourcesAndTasks.TYPE_PROPERTY_VALUE_HUMAN))
							return true;
						else
							break;
					}
		...
		for (AssignmentDataModel anAssignment : existingAssignments) {
			if (resourceId.equals(anAssignment.getTaskDataModel().getTaskId())) {
				...
				String angle = anAssignment.getResourceDataModel()
						.getProperty(MapToResourcesAndTasks.ROTATION_Z_PROPERTY_NAME);
				Point resourcePoint = DataModelToAWTHelper.getPointFromResource(anAssignment.getResourceDataModel());
				if (!angle.equals("180") && !angle.equals("360") && !angle.equals("0"))
					suitableResourceRectangle = DataModelToAWTHelper.rotateShape(Integer.parseInt(angle),
							suitableResourceRectangle, resourcePoint.x, resourcePoint.y,
							anAssignment.getTaskDataModel().getProperty(MapToResourcesAndTasks.SHAPE_TYPE_NAME));
				int reachability = Integer.parseInt(
						anAssignment.getTaskDataModel().getProperty(MapToResourcesAndTasks.REACHABILTY_PROPERTY_NAME));
				...
				Dimension reachabilityDimension = new Dimension(reachability + aDimension.width,
						reachability + aDimension.height);
				...
				if (taskRectangle.intersects(reachabilityRectangle)
						&& !taskRectangle.intersects(suitableResourceRectangle.getBounds()))
					areWithinReachability = true;
				else
					areWithinReachability = false;
